...
| `npm run test:unit`         | Vitest `unit` project: the browserless `*.test.js` files beside `components/` and `js/` (pure helpers).          | After editing a helper that has a `.test.js`.         |
| `npm run test:ci`           | `format:check && lint && type-check && lint:jsdoc && lint:lit-analyzer && test:unit`. What `frontend_lint` runs. | Before pushing.                                       |
