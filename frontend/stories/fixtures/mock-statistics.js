 ...
 * page's interesting behaviors are all reachable from it:
 *
 * - more families with data (10) than categorical color slots (7), so the
 ...
 *
 * The SERVER slices, so a fixture that ignores the query would
 ...
 * weekly view is recognizably the same database as the monthly one.
 ...
 * color and fills the family select.
 ...
 * with each dimension's own filter left out, and the tiles, storage,
 * heatmap, hosts and unresolved plans are never filtered at all.
 *
 * Variant and certification filters halve every series rather than modeling
