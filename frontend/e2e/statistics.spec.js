 ...
 *
 * The SERVER slices, so these specs assert first and foremost on
 ...
 * colors and the family select's options come from the all-time
...
/**
 * Mirrors the endpoint's refresh behavior: a forced recompute answers 200
...
test.describe("statistics.html — admin usage dashboard", () => {
  ...
  test("each range preset asks the server for the axis it names", async ({ page }) => {
    ...
    // which carries the color ranking and the family options itself.
    expect(searches).toHaveLength(1);
  ...
  test("a narrowed view is one request, and still offers every family", async ({ page }) => {
    ...
    // ...are one request: the color ranking and the family options ride on
  ...
  test("a drill-down names the slice behind the mark that was activated", async ({ page }) => {
    ...
    // Canceling the event is what keeps the page here; production has no
