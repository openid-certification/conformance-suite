...
test.describe("login.html — Login page", () => {
  ...
  test("renders both OAuth cts-link-button elements with correct href and label", async ({
    ...
    const googleBtn = page.locator(
      'cts-link-button[href="/oauth2/authorization/google"]',
    );
    const gitlabBtn = page.locator(
      'cts-link-button[href="/oauth2/authorization/gitlab"]',
    );
  ...
  test("both OAuth buttons render at size=lg (inner anchor has btn-lg)", async ({
    page,
  }) => {
    await setupFailFast(page);
    await setupLoginRoutes(page);
    ...
    const googleAnchor = page.locator(
      'cts-link-button[href="/oauth2/authorization/google"] a',
    );
    const gitlabAnchor = page.locator(
      'cts-link-button[href="/oauth2/authorization/gitlab"] a',
    );
  ...
  test("Google button uses variant=danger (inner anchor has btn-danger)", async ({
    page,
  }) => {
    await setupFailFast(page);
    await setupLoginRoutes(page);
    ...
    await expect(googleAnchor).toHaveAttribute(
      "href",
      "/oauth2/authorization/google",
    );
  });

  test("GitLab button uses variant=primary (inner anchor has btn-primary)", async ({
    page,
  }) => {
    await setupFailFast(page);
    await setupLoginRoutes(page);
    ...
    await expect(gitlabAnchor).toHaveAttribute(
      "href",
  ...
  test("cts-navbar renders in unauthenticated state", async ({ page }) => {
    ...
    await expect(
      navbar.locator('a[href="plans.html?public=true"]'),
    ).toBeVisible();
  });
});
