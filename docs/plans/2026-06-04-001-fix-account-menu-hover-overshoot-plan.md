...
- **Test scenarios:** (one consistent measurement basis: `getBoundingClientRect()` on both the item and the menu; tolerance ±2px absorbs sub-pixel rounding)
  ...
  - Edge case: render the story with a long principal (e.g., a 60+ character email) so the menu grows past its `min-width: 240px`; repeat the containment and fill assertions at the wider size.
