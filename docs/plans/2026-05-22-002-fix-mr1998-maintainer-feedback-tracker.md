...
| open | B6 | almgren | TBD | — | — |
| open | B7 | almgren | TBD | — | — |
| closed | C1 | thomasdarimont | [#note_3371528678](https://gitlab.com/openid/conformance-suite/-/merge_requests/1998#note_3371528678) (label-consistency portion) | `dc7e053a3` | "Action label consistency landed in `dc7e053a3` (U3): the status-bar primaries now read 'Start Test' (WAITING) and 'Repeat Test' (FINISHED), matching cts-test-nav-controls' 'Repeat Test' / 'Continue Plan' so every action follows the `<Verb> <Object>` convention. The HTTP 400 portion of this thread (A3) is still open and ships in U4." |
