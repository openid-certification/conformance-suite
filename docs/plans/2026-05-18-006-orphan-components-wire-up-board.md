...
| 2 | [—] | `cts-stat` | ~~`cts-dashboard` (`index.html`)~~ — deleted | [`008-feat-wire-cts-stat-into-dashboard-plan.md`](2026-05-18-008-feat-wire-cts-stat-into-dashboard-plan.md) | Removed — wiring retired with the dashboard |

Wired then deleted. Shipped 05df5c786 (2026-05-19) as a 4-tile stats row on `<cts-dashboard>` (`index.html`). The plans-page-as-home redesign retired that dashboard (`30a64c00e`, 2026-05-29) — deleting `index.html` and re-orphaning the tile, which mounted on no page thereafter. Deleted 2026-06-08 on feat/redesign: removed `cts-stat.js`, its Storybook story, and the dead `cts-stat:not(:defined)` FOUC rule in `css/layout.css`. The SUS "current state of system" recommendation it once mapped to is now a Product decision on the decisions board — to be answered by building a purpose-fit status surface if/when one is scheduled, not by parking orphaned inventory. Original wiring detail preserved in plan `008` and `docs/residual-review-findings/2026-05-19-cts-stat-dashboard-05df5c786.md`.
...
| 6 | [—] | `cts-batch-runner` | ~~`plan-detail.html`~~ (descoped) | [`012-feat-wire-cts-batch-runner-into-plan-detail-plan.md`](2026-05-18-012-feat-wire-cts-batch-runner-into-plan-detail-plan.md) | Descoped — no analogue in master; awaits team validation |

Descoped (2026-05-19): page wiring removed from `plan-detail.html` pending product validation. `master` has zero batch-running functionality (verified: no `cts-batch-runner.js`, no `batchRunner`/`dispatchBatch`/`runTestNoNavigate` references in `plan-detail.html`). Batch-run was a forward-looking design exploration on `feat/redesign`, not a feature parity item — until Joseph or Thomas validates it as a desirable workflow, the right default is to ship without it. The component itself (`cts-batch-runner.js` + `cts-batch-runner.stories.js`) stays intact as a Storybook-only design citizen; only the page consumer was removed. If validated later, re-wiring is the plan's U1 again (~50 LOC) plus the maintainer-decision work documented in `tmp/row-6-decision.html`.

Previously shipped and now reverted: U1 wiring `645ee04d3` (feat) + `ea679b7b0` (residual findings) via draft MR [!1998](https://gitlab.com/openid/conformance-suite/-/merge_requests/1998). All eight maintainer-decision residuals from `docs/residual-review-findings/2026-05-19-cts-batch-runner-plan-detail-645ee04d3.md` are moot for now — they were gated on decisions that no longer need to be made. Close MR !1998 with a pointer to this descope note.
