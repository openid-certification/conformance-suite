...
public class ValidateErrorFromTokenEndpointResponseError_UnitTest {
	...
	/**
	 * Test method is invalid by pattern [\x20-\x21\x23-\x5B\x5D-\x7E]+
	 */
	@Test
	public void testIsValidFieldFormat_Invalid() {
		...
		for (int i = HEX_X00_ASCII; i <= HEX_X1F_ASCII; i++) {
			assertThat(cond.isValidFieldFormat(str + (char) i, ERROR_FIELD_PATTERN_VALID)).isFalse();
		}

		// Test method by input string which has value with suffix include character %X22 (HEX)
		int HEX_X22_ASCII = 34;
		assertThat(cond.isValidFieldFormat(str + (char) HEX_X22_ASCII, ERROR_FIELD_PATTERN_VALID)).isFalse();

		// Test method by input string which has value with suffix include character %X5C (HEX)
		int HEX_X5C_ASCII = 92;
		assertThat(cond.isValidFieldFormat(str + (char) HEX_X5C_ASCII, ERROR_FIELD_PATTERN_VALID)).isFalse();
	...
	/**
	 * Test method is valid by pattern [\x20-\x21\x23-\x5B\x5D-\x7E]+
	 */
	@Test
	public void testIsValidFieldFormat_Valid() {
		...
		for (int i = HEX_X20_ASCII; i <= HEX_X21_ASCII; i++) {
			assertThat(cond.isValidFieldFormat(str + (char) i, ERROR_FIELD_PATTERN_VALID)).isTrue();
		}
