...
public class ValidateConfiguredClientCertificatesAgainstRical_UnitTest {
	...
	public void testEvaluate_failsWhenListedEntryIsNotATrustAnchor() {
		// F.3.2.6: a chain reaching only isTrustAnchor=false entries is not covered, even though
		// multipaz's RicalTrustManager makes a trust point of every listed entry
		RicalTestFixtures.putClientJwks(env, pki);
	...
	public void testEvaluate_failsWhenRicalNotAfterHasPassed() {
		// F.3.2.5: an expired list's entries cannot be used as trust anchors
		RicalTestFixtures.putClientJwks(env, pki);
		RicalTestFixtures.putRical(env, RicalTestFixtures.sign(RicalTestFixtures.buildRicalMap(
	...
	public void testEvaluate_failsWhenJwksHasNoX5c() {
		// a usable signing key, but without the certificate chain the RICAL check needs
		RicalTestFixtures.putClientJwks(env, pki, "client_jwks", false);
		RicalTestFixtures.putRical(env, RicalTestFixtures.goodSignedRical(List.of(pki.getCaCert())));
