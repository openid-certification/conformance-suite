...
object RicalTestFixtures {
	 ...
	 * Identifier) but is signed by a fresh, unrelated key: a chain that lies about its issuer.
	 */
	@JvmStatic
	fun impostorReaderCert(claimedIssuer: X509Cert): X509Cert {
		val rogueKey = runBlocking { Crypto.createEcPrivateKey(EcCurve.P256) }
		val readerKey = runBlocking { Crypto.createEcPrivateKey(EcCurve.P256) }
		return runBlocking {
			X509Cert.Builder(
				readerKey.publicKey,
				AsymmetricKey.anonymous(rogueKey, Algorithm.ES256),
				ASN1Integer(3L),
	...
	fun sign(
		...
		// the signer key is P-256, so only algorithms for that curve produce a valid signature
