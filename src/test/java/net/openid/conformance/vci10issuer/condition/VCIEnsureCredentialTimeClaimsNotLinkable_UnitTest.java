...
public class VCIEnsureCredentialTimeClaimsNotLinkable_UnitTest {
	...
	/** Set an additional time claim (exp/nbf) on an SD-JWT capture's issuer JWT claims. */
	private void setClaim(JsonObject sdJwtEntry, String name, long value) {
		sdJwtEntry.getAsJsonObject("sdjwt").getAsJsonObject("credential").getAsJsonObject("claims")
	...
	@Test
	public void expTracksWhileIatRounded_fails() {
		// iat is rounded (identical) but exp tracks the real gap => still a linkability failure.
		JsonObject first = sdJwtCapture(86400L, 1000, false);
		setClaim(first, "exp", 1_000_000L);
		JsonObject second = sdJwtCapture(86400L, 1005, false);
		setClaim(second, "exp", 1_000_005L);
		put(first, second);
		assertThrows(ConditionError.class, () -> cond.execute(env));
	}

	@Test
	public void nbfTracks_fails() {
		JsonObject first = sdJwtCapture(86400L, 1000, false);
		setClaim(first, "nbf", 500L);
		JsonObject second = sdJwtCapture(86400L, 1005, false);
		setClaim(second, "nbf", 505L);
		put(first, second);
		assertThrows(ConditionError.class, () -> cond.execute(env));
	}

	@Test
	public void allTimeClaimsRoundedOrIdentical_passes() {
		// iat, exp and nbf all identical across the two credentials => none track the gap.
		JsonObject first = sdJwtCapture(86400L, 1000, false);
		setClaim(first, "exp", 1_000_000L);
		setClaim(first, "nbf", 86400L);
		JsonObject second = sdJwtCapture(86400L, 1005, false);
		setClaim(second, "exp", 1_000_000L);
		setClaim(second, "nbf", 86400L);
		put(first, second);
		cond.execute(env);
	}
