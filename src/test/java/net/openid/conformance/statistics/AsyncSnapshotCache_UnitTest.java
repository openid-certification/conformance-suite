...
class AsyncSnapshotCache_UnitTest {
	...
	@Test
	void forceWhileTheComputationRunsHasItRunAgainOnceItFinishes() {
		AtomicInteger computations = new AtomicInteger();
		computation.set(() -> "run " + computations.incrementAndGet());
		cache.get(false);
		...
		executor.runAll();
		// the second run reads whatever the caller refreshed for; a third would be a waste
		assertThat(computations.get()).isEqualTo(2);
		assertThat(ready(cache.get(false)).value()).isEqualTo("run 2");
		assertThat(ready(cache.get(false)).refreshing()).isFalse();
	}

	@Test
	void anUnforcedCallWhileTheComputationRunsDoesNotHaveItRunAgain() {
		AtomicInteger computations = new AtomicInteger();
		computation.set(() -> "run " + computations.incrementAndGet());
		cache.get(false);
	...
	@Test
	void anErrorThrownByTheComputationIsAFailureWithABackoffLikeAnyOther() {
		computation.set(() -> {
		...
		AsyncSnapshotCache.Failed<String> failed = failed(cache.get(false));
		assertThat(failed.failure().message()).isEqualTo("Java heap space");
		assertThat(executor.submitCount()).as("not retried inside the backoff window").isEqualTo(1);
	}
