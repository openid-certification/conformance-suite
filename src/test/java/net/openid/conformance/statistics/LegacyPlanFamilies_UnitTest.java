...
class LegacyPlanFamilies_UnitTest {
	...
	/** @return the plan names the registry publishes now, as the suite's own registry scan finds them */
	private static Set<String> publishedNow() {
		Set<String> names = new TreeSet<>();
		for (VariantService.TestPlanHolder plan : new VariantService(holder -> true).getTestPlans()) {
			names.add(plan.info.testPlanName());
		}
		return names;
	}
