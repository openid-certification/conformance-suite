...
class FAPI2MessageSigningFinalGrantManagementApplicability_UnitTest {
	...
	private static Map<String, String> serverVariant(String profile) {
		Map<String, String> variant = new HashMap<>();
		variant.put("fapi_profile", profile);
		variant.put("client_auth_type", "private_key_jwt");
		variant.put("sender_constrain", "dpop");
		variant.put("openid", "plain_oauth");
		variant.put("fapi_response_mode", "plain_response");
	...
	void grantManagementModulesAreOnlyApplicableForGenericFapiAndChile() {
		...
		for (String profile : new String[] { "plain_fapi", "openbanking_chile" }) {
			...
			for (String moduleName : GM_MODULE_NAMES) {
				assertThat(variantService.getTestModule(moduleName).isApplicableForVariant(selection))
					.as("%s must be applicable for %s with grant management enabled", moduleName, profile)
					.isTrue();
			}
		}

		for (String profile : new String[] { "openbanking_uk", "openbanking_brazil", "consumerdataright_au",
			"connectid_au", "cbuae", "ksa", "vci", "vci_haip" }) {
			VariantSelection selection = new VariantSelection(serverVariant(profile));

			for (String moduleName : GM_MODULE_NAMES) {
				assertThat(variantService.getTestModule(moduleName).isApplicableForVariant(selection))
					.as("%s must not be applicable for %s", moduleName, profile)
					.isFalse();
			}
		}
	}
