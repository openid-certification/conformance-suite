...
public class TrustChainValidator_UnitTest {
	...
	private String createDummyJWT(String issuer, String subject, String key) throws ParseException, JOSEException {
		...
		JWSSigner signer = jwsSignerFactory.createJWSSigner(jwk, alg);
	...
	@Test
	void testValidateTrustChain_emptyChain() {
		List<String> emptyChain = new ArrayList<>();
		String relyingParty = "https://rp.example.com";

		TrustChainValidator.ValidationResult result = TrustChainValidator.validateTrustChain(emptyChain, relyingParty);

		assertThat(result.isValid()).isFalse();
		assertThat(result.getErrors()).isEqualTo("Trust chain is null or empty.");
	}

	@Test
	void testValidateTrustChain_singleToken() throws ParseException, JOSEException {
		List<String> singleTokenChain = new ArrayList<>();
		String relyingParty = "https://rp.example.com";
		String token = createDummyJWT(relyingParty, relyingParty, LEAF_KEY);
		singleTokenChain.add(token);

		TrustChainValidator.ValidationResult result = TrustChainValidator.validateTrustChain(singleTokenChain, relyingParty);

		assertThat(result.isValid()).isFalse();
		assertThat(result.getErrors()).contains("Trust chain must contain at least two tokens.");
	}


	@Test
	void testValidateTrustChain_rpConfigNotSelfSigned() throws Exception {
		List<String> trustChain = new ArrayList<>();
		String relyingParty = "https://rp.example.com";
		String rpToken = createDummyJWT("https://different-issuer.example.com", relyingParty, LEAF_KEY);
		String taToken = createDummyJWT("https://trust-anchor.example.com", "https://trust-anchor.example.com", TRUST_ANCHOR_KEY);
		trustChain.add(rpToken);
		trustChain.add(taToken);

		TrustChainValidator.ValidationResult result = TrustChainValidator.validateTrustChain(trustChain, relyingParty);
	...
	@Test
	void testValidateTrustChain_rpIdentifierMismatch() throws Exception {
		List<String> trustChain = new ArrayList<>();
		String relyingParty = "https://rp.example.com";
		String mismatchedRp = "https://different-rp.example.com";

		// Create a chain with mismatched RP identifier
		String token0 = createDummyJWT(mismatchedRp, mismatchedRp, LEAF_KEY);
		String token1 = createDummyJWT("https://org-a.example.com", mismatchedRp, INTERMEDIATE_KEY);
		String token2 = createDummyJWT("https://trust-anchor.example.com", "https://org-a.example.com", TRUST_ANCHOR_KEY);
		String token3 = createDummyJWT("https://trust-anchor.example.com", "https://trust-anchor.example.com", TRUST_ANCHOR_KEY);

		trustChain.add(token0);
		trustChain.add(token1);
		trustChain.add(token2);
		trustChain.add(token3);
	...
	@Test
	void testValidateTrustChain_subordinateStatementMismatch() throws Exception {
		List<String> trustChain = new ArrayList<>();
		String relyingParty = "https://rp.example.com";
		String orgA = "https://org-a.example.com";

		// RP's self-signed entity configuration
		trustChain.add(createDummyJWT(relyingParty, relyingParty, LEAF_KEY));

		// Subordinate statement about RP (with mismatched subject)
		trustChain.add(createDummyJWT(orgA, "https://wrong-rp.example.com", INTERMEDIATE_KEY));
	...
	@Test
	void testValidateTrustChain_subjectMismatch() throws Exception {
		List<String> trustChain = new ArrayList<>();
		String relyingParty = "https://rp.example.com";
		String orgA = "https://org-a.example.com";
		String trustAnchor = "https://trust-anchor.example.com";

		// RP self-signed
		trustChain.add(createDummyJWT(relyingParty, relyingParty, LEAF_KEY));
		// Statement about RP by Org A
		trustChain.add(createDummyJWT(orgA, relyingParty, INTERMEDIATE_KEY));
		// Statement about Org A by Trust Anchor, but with mismatched subject
		trustChain.add(createDummyJWT(trustAnchor, "https://wrong-org.example.com", TRUST_ANCHOR_KEY));
		// Trust Anchor self-signed
		trustChain.add(createDummyJWT(trustAnchor, trustAnchor, TRUST_ANCHOR_KEY));
	...
	@Test
	void testValidateTrustChain_trustAnchorNotSelfSigned() throws Exception {
		List<String> trustChain = new ArrayList<>();
		String relyingParty = "https://rp.example.com";
		String orgA = "https://org-a.example.com";
		String trustAnchor = "https://trust-anchor.example.com";
		String differentIssuer = "https://different-issuer.example.com";

		// RP self-signed configuration
		trustChain.add(createDummyJWT(relyingParty, relyingParty, LEAF_KEY));
		// Subordinate statement about RP
		trustChain.add(createDummyJWT(orgA, relyingParty, INTERMEDIATE_KEY));
		// Subordinate statement about Org A
		trustChain.add(createDummyJWT(trustAnchor, orgA, TRUST_ANCHOR_KEY));
		// Trust Anchor configuration (not self-signed)
		trustChain.add(createDummyJWT(differentIssuer, trustAnchor, TRUST_ANCHOR_KEY));
	...
	@Test
	void testValidateTrustChain_trustAnchorIdentifierMismatch() throws Exception {
		List<String> trustChain = new ArrayList<>();
		String relyingParty = "https://rp.example.com";
		String orgA = "https://org-a.example.com";
		String trustAnchor = "https://trust-anchor.example.com";
		String mismatchedIssuer = "https://mismatched-issuer.example.com";

		// RP self-signed
		trustChain.add(createDummyJWT(relyingParty, relyingParty, LEAF_KEY));
		// Subordinate statement about RP by Org A
		trustChain.add(createDummyJWT(orgA, relyingParty, INTERMEDIATE_KEY));
		// Subordinate statement about Org A by mismatched issuer
		trustChain.add(createDummyJWT(mismatchedIssuer, orgA, TRUST_ANCHOR_KEY));
		// Trust Anchor self-signed (doesn't match previous issuer)
		trustChain.add(createDummyJWT(trustAnchor, trustAnchor, TRUST_ANCHOR_KEY));
	...
	/*
	@Test
	void testValidateTrustChain_invalidSignature() {
		List<String> trustChain = new ArrayList<>();
		String relyingParty = "https://rp.example.com";
		String rpToken = createDummyJWT(relyingParty, relyingParty);
		String taToken = createDummyJWT("https://trust-anchor.example.com", "https://trust-anchor.example.com");
		trustChain.add(rpToken);
		trustChain.add(taToken);

		validator = new TrustChainValidator();
		validator.setProperties("UNIT-TEST", eventLog, ConditionResult.INFO);

		// Mock the verifySignature method to return false for the second token
		Mockito.when(validator.verifySignature(rpToken)).thenReturn(true);
		Mockito.when(validator.verifySignature(taToken)).thenReturn(false);

		boolean result = validator.validateTrustChain(trustChain, relyingParty);

		assertThat(result).isFalse();
		verify(eventLog).log(ArgumentMatchers.eq("Signature verification failed for token at index 1"));
	}
	*/

	/*
	@Test
	void testValidateTrustChain_claimValidationFails() {
		List<String> trustChain = new ArrayList<>();
		String relyingParty = "https://rp.example.com";
		String rpToken = createDummyJWT(relyingParty, relyingParty);
		String taToken = createDummyJWT("https://trust-anchor.example.com", "https://trust-anchor.example.com");
		...
		// Mock the verifyClaims method to return false for the first token
		Mockito.when(validator.verifyClaims(Mockito.any())).thenReturn(false);

		boolean result = validator.validateTrustChain(trustChain, relyingParty);

		assertThat(result).isFalse();
		verify(eventLog).log(ArgumentMatchers.eq("Claim validation failed for token at index 0"));
	}
	 */
