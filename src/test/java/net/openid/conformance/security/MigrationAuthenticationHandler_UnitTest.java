...
public class MigrationAuthenticationHandler_UnitTest {
	...
	public void setUp() {
		...
		savedConfigurationService = Mockito.mock(SavedConfigurationService.class);
		handler = new MigrationAuthenticationHandler(testPlanService, testInfoService, tokenService,
	...
	public void migrates_ownership_when_the_legacy_identity_claims_are_present() throws Exception {
		...
		Mockito.when(tokenService.migrateOwnership(LEGACY_ISS, LEGACY_SUB)).thenReturn(3L);
		Mockito.when(savedConfigurationService.migrateOwnership(LEGACY_ISS, LEGACY_SUB)).thenReturn(1L);

		handler.onAuthenticationSuccess(request, response,
			oidcLoginWithClaims(Map.of("idp_iss", LEGACY_ISS, "idp_sub", LEGACY_SUB)));

		// Every owner-scoped store has to move together. Migrating only some of them
		// leaves the user's records split across two identities, which reads as data
		// loss from the UI and is not something the user can fix themselves.
		...
		Mockito.verify(tokenService).migrateOwnership(LEGACY_ISS, LEGACY_SUB);
		Mockito.verify(savedConfigurationService).migrateOwnership(LEGACY_ISS, LEGACY_SUB);
	...
	public void does_not_migrate_when_only_one_of_the_legacy_claims_is_present() throws Exception {
		...
		Mockito.verifyNoInteractions(testPlanService, testInfoService, tokenService, savedConfigurationService);
		Assertions.assertNotNull(response.getRedirectedUrl(), "login must still complete");
	}
