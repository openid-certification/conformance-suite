...
export const EmptyForm = {
  ...
  async play({ canvasElement, step }) {
    ...
    await step("container carries the OIDF namespace so scoped CSS rules apply", async () => {
      expect(canvasElement.querySelector(".oidf-config-form")).toBeTruthy();
    });

    await step("tabs render the OIDF tablist with Form active by default", async () => {
    ...
    await step("sections render with OIDF markup and overline-style legends", async () => {
    ...
    await step("field labels delegated to cts-form-field still render", async () => {
      expect(canvas.getByText("Issuer URL")).toBeTruthy();
      expect(canvas.getByText("Client ID")).toBeTruthy();
    });

    await step("validate action delegates to cts-button — inner button renders", async () => {
    ...
    await step("no Bootstrap form-* classes leak through into the rendered DOM", async () => {
...
export const PrefilledForm = {
  ...
  `,
  async play({ canvasElement, step }) {
    await step("issuer URL input reflects the bound config value", async () => {
    ...
    await step("client ID input reflects the bound config value", async () => {
...
export const JsonTab = {
  ...
  async play({ canvasElement, step }) {
    ...
    await step("clicking the JSON tab activates it", async () => {
    ...
    await step("editor renders the config as JSON with no error state", async () => {
...
export const ValidJsonDispatchesConfigChange = {
  ...
  async play({ canvasElement, step }) {
    ...
    await step(
      "a valid JSON edit dispatches cts-config-change with the parsed config",
      async () => {
    ...
    await step("the success branch leaves the editor free of error state", async () => {
...
export const InvalidJsonShowsError = {
  ...
  async play({ canvasElement, step }) {
    ...
    await step("invalid JSON puts the editor into its error state", async () => {
    ...
    await step("the inline json-error callout renders", async () => {
...
export const WithValidationErrors = {
  ...
  async play({ canvasElement, step }) {
    ...
    await step("errors roll up from cts-form-field via the .oidf-error class", async () => {
...
export const ValidateButtonShowsSuccessMessage = {
  ...
  async play({ canvasElement, step }) {
    ...
    await step("clicking validate opens the feedback window", async () => {
    ...
    await step("cts-validate fires with valid=true and an empty errors map", async () => {
      await waitFor(() => {
    ...
    await step("the inline success verdict lands as the spinner stops", async () => {
...
export const ValidateButtonShowsErrorMessage = {
  ...
  async play({ canvasElement, step }) {
    ...
    await step("cts-validate fires valid=false with an errors map for required paths", async () => {
      // The non-required `alias.optional` is NOT flagged.
      await waitFor(() => {
        expect(validateEvent).not.toBeNull();
      }, VERDICT_WAIT);
      /** @type {any} */
    ...
    await step("inline error verdict uses plural count copy for two missing fields", async () => {
      const verdict = canvasElement.querySelector('[data-testid="validate-verdict"]');
    ...
    await step("inline .oidf-error callouts land next to the offending inputs", async () => {
      await waitFor(() => {
        const errors = canvasElement.querySelectorAll(".oidf-error");
        expect(errors.length).toBe(2);
      });
...
export const ValidateVerdictClearsOnConfigChange = {
  ...
  async play({ canvasElement, step }) {
    ...
    await step("validating renders the first verdict", async () => {
      await userEvent.click(button);
      await waitFor(() => {
        expect(verdict.textContent).toContain("Configuration is valid");
      }, VERDICT_WAIT);
    ...
    await step("a Form-tab field edit clears the verdict", async () => {
      const issuerInput = canvasElement.querySelector('input[type="url"]');
    ...
    await step("a JSON-tab edit clears the verdict (but a bare tab switch does not)", async () => {
      // A bare tab switch does NOT clear the verdict — validation reads
      // `this.config`, which both tabs keep in sync — only the edit does.
      await userEvent.click(button);
      await waitFor(() => {
        expect(verdict.textContent).toContain("Configuration is valid");
      }, VERDICT_WAIT);
    ...
    await step("a programmatic .config reassignment clears the verdict", async () => {
      await userEvent.click(button);
      await waitFor(() => {
        expect(verdict.textContent).toContain("Configuration is valid");
      }, VERDICT_WAIT);
...
export const ValidateCancelledByMidWindowEdit = {
  ...
  async play({ canvasElement, step }) {
    ...
    await step("clicking validate opens the loading window", async () => {
      await userEvent.click(button);
      await waitFor(() => {
        expect(host.hasAttribute("loading")).toBe(true);
      });
    ...
    await step("a mid-window field edit aborts the in-flight validation", async () => {
      // The button settles back out of its loading state.
      const issuerInput = canvasElement.querySelector('input[type="url"]');
    ...
    await step("the cancelled timer never fires past the original window", async () => {
...
export const WithHiddenFields = {
  ...
  async play({ canvasElement, step }) {
    ...
    await step("Form tab: client_id renders, hidden client_secret does not", async () => {
    ...
    await step("sections that still have visible fields render normally", async () => {
      const legends = canvasElement.querySelectorAll("legend.oidf-config-form-section-title");
      expect(legends.length).toBe(2);
    });

    await step("JSON-tab pretty-print omits the hidden key", async () => {
      // Full submit shape still preserves it via the cts-config-change merge.
      await userEvent.click(canvas.getByRole("tab", { name: "JSON" }));
      const editor = await waitForJsonEditor(canvasElement);
      const json = JSON.parse(/** @type {any} */ (editor).value);
    ...
    await step("editing JSON emits the full config with the hidden key merged back", async () => {
      const editor = await waitForJsonEditor(canvasElement);
    ...
    await step("a Form-tab edit also preserves the hidden value", async () => {
      // _handleFieldChange runs structuredClone(this.config), _setAtPath the
      // new value, and emits — no merge step is needed because the clone
      // carries the hidden key forward. This is a distinct code path from
      // the JSON-tab merge above; a future refactor that constructs the
      // config from scratch in _handleFieldChange would break it.
      receivedConfig = null;
      await userEvent.click(canvas.getByRole("tab", { name: "Form" }));
...
export const ObjectFieldRoundTrip = {
  ...
  async play({ canvasElement, step }) {
    ...
    await step(
      "the object value displays as pretty-printed JSON, not [object Object]",
      async () => {
        // Verifies cts-form-field._displayValue formats type:object values.
    ...
    await step("editing to valid JSON dispatches a parsed OBJECT, not a string", async () => {
      // The page's currentConfig listener (and the eventual JSON.stringify
      // on submit) sees the parsed shape.
...
export const ExplicitFieldsMode = {
  ...
  `,
  async play({ canvasElement, step }) {
    await step("four sections render, one field each", async () => {
      const legends = canvasElement.querySelectorAll("legend.oidf-config-form-section-title");
    ...
    await step(
      "a cross-prefix field composes to its full data path, not section+path",
      async () => {
        // A section keyed "client_tls_auth" rendering a field whose data path
        // lives under client.* — the composed field name must be the full
        // path, not "client_tls_auth.client.tls_client_auth_…".
...
export const ExternalConfigUpdatesFormFields = {
  ...
  async play({ canvasElement, step }) {
    ...
    await step("pre-condition: empty form, no values bound to inputs", async () => {
      const issuerInputInitial = canvasElement.querySelector('input[type="url"]');
      expect(issuerInputInitial.value).toBe("");
    });

    await step("reassigning .config from outside updates the Form-tab inputs", async () => {
      form.config = {
...
export const ExternalConfigUpdatesJsonEditor = {
  ...
  async play({ canvasElement, step }) {
    ...
    await step("the JSON editor seeds with the initial config", async () => {
      const initial = JSON.parse(editor.value);
      expect(initial.server.issuer).toBe("https://old.example.com");
    });

    await step("reassigning .config after activation re-renders the editor body", async () => {
      form.config = {
        server: { issuer: "https://loaded.example.com" },
        client: { client_id: "loaded-client" },
      };
