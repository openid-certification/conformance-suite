 ...
 */
// --- Status dot (independent of the chip fill) ---
//
// A neutral `secondary` chip can carry a colored status dot so the plans
// listing keeps the module name legible while re-introducing per-module
// status color. The dot color is independent of the chip fill. The
// `pending` dot is a gray pulsing placeholder (capped at 10 iterations,
// gated behind prefers-reduced-motion) that recolors when status resolves.

/**
 * A neutral `secondary` chip with a `pass` status dot. The chip fill stays
 * neutral (the module name must read clearly) while the dot carries the
 * color. The dot is decorative (`aria-hidden`) — the chip text/title is
 * the accessible carrier.
 */
export const WithStatusDot = {
  args: { variant: "secondary", label: "oidcc-server", dot: true, "dot-variant": "pass" },
  render: ({ variant, label }) =>
    html`<cts-badge variant="${variant}" label="${label}" dot dot-variant="pass"></cts-badge>`,

  async play({ canvasElement }) {
    const badge = canvasElement.querySelector(".badge");
    expect(badge).toBeTruthy();
    // Chip fill is unchanged by the dot color.
    expect(badge.classList.contains("b-secondary")).toBe(true);
    expect(badge.textContent.trim()).toBe("oidcc-server");

    const dot = badge.querySelector(".cts-badge-dot");
    expect(dot).toBeTruthy();
    expect(dot.classList.contains("cts-badge-dot-pass")).toBe(true);
    // Decorative — accessible meaning comes from the chip text/title.
    expect(dot.getAttribute("aria-hidden")).toBe("true");
  },
};

/** No `dot` attribute → no dot element rendered. */
export const NoDotByDefault = {
  args: { variant: "secondary", label: "oidcc-server" },
  render: ({ variant, label }) =>
    html`<cts-badge variant="${variant}" label="${label}"></cts-badge>`,

  async play({ canvasElement }) {
    const badge = canvasElement.querySelector(".badge");
    expect(badge.querySelector(".cts-badge-dot")).toBeNull();
  },
};

/**
 * Each concrete dot-variant maps to its scoped color class; an unknown
 * variant renders the base neutral dot (no modifier class), never an
 * unstyled element.
 */
export const DotVariants = {
  render: () => html`
    <div style="display: flex; flex-wrap: wrap; gap: 0.5rem; padding: 1rem;">
      ${["pass", "fail", "warn", "running", "skip", "review", "info", "pending", "bogus"].map(
        (v) =>
          html`<cts-badge variant="secondary" label="${v}" dot dot-variant="${v}"></cts-badge>`,
      )}
    </div>
  `,

  async play({ canvasElement }) {
    const dots = canvasElement.querySelectorAll(".cts-badge-dot");
    ...
    const order = ["pass", "fail", "warn", "running", "skip", "review", "info", "pending", "bogus"];
    order.forEach((v, i) => {
      if (v === "bogus") {
        // Unknown → base class only, no modifier, but still a defined fill.
        expect(dots[i].className).toBe("cts-badge-dot");
        const bg = window.getComputedStyle(dots[i]).backgroundColor;
        expect(bg).not.toBe("rgba(0, 0, 0, 0)");
      } else {
        expect(dots[i].classList.contains(expected[v])).toBe(true);
      }
    });
  },
};
...
/**
 * The pending dot pulses, capped at 10 iterations ("gray and pulse, 10
 * times max"). The assertion adapts to the runner's motion preference: in
 * the default no-preference environment the animation is active and
 * iteration-count resolves to 10; under prefers-reduced-motion the gate
 * suppresses it to `none`. Either outcome is correct — both are asserted
 * so the test is not flaky across environments.
 */
export const PendingDotPulseCapped = {
  args: { variant: "secondary", label: "oidcc-server", dot: true, "dot-variant": "pending" },
  render: ({ variant, label }) =>
    html`<cts-badge variant="${variant}" label="${label}" dot dot-variant="pending"></cts-badge>`,

  async play({ canvasElement }) {
    const dot = canvasElement.querySelector(".cts-badge-dot");
    expect(dot).toBeTruthy();
    expect(dot.classList.contains("cts-badge-dot-pending")).toBe(true);

    const computed = window.getComputedStyle(dot);
    const reduced = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    if (reduced) {
      // Gate suppresses the pulse for motion-sensitive users.
      expect(computed.animationName).toBe("none");
    } else {
      // Capped at 10 iterations — the load-bearing "10 times max" rule.
      expect(computed.animationName).toContain("cts-badge-dot-pulse");
      expect(computed.animationIterationCount).toBe("10");
    }
  },
};

/**
 * Late-resolve recolor: flipping `dot-variant` from `pending` to a concrete
 * value re-renders the dot WITHOUT the pending/pulse class. Because
 * cts-badge is attribute-driven (the recolor fires from
 * attributeChangedCallback → _render, not from an `animationend` listener),
 * a fetch that resolves AFTER the 10-iteration pulse has stopped still
 * recolors correctly.
 */
export const DotRecolorDropsPulse = {
  args: { variant: "secondary", label: "oidcc-server", dot: true, "dot-variant": "pending" },
  render: ({ variant, label }) =>
    html`<cts-badge variant="${variant}" label="${label}" dot dot-variant="pending"></cts-badge>`,
  ...
  async play({ canvasElement }) {
    const host = canvasElement.querySelector("cts-badge");
    let dot = canvasElement.querySelector(".cts-badge-dot");
    expect(dot.classList.contains("cts-badge-dot-pending")).toBe(true);

    // Simulate the status fetch resolving to PASSED.
    host.setAttribute("dot-variant", "pass");

    dot = canvasElement.querySelector(".cts-badge-dot");
    expect(dot.classList.contains("cts-badge-dot-pass")).toBe(true);
    expect(dot.classList.contains("cts-badge-dot-pending")).toBe(false);
    // No residual pulse animation on the resolved dot.
    expect(window.getComputedStyle(dot).animationName).toBe("none");
  },
};

/**
 * The dot coexists with the existing affordance ring and the running
 * spinner — a regression guard since cts-badge is shared app-wide.
 */
export const DotCoexistsWithRingAndSpinner = {
  render: () => html`
    <div style="display: flex; gap: 0.5rem; padding: 1rem;">
      <cts-badge
        ...
        dot-variant="fail"
      ></cts-badge>
      <cts-badge variant="running" label="running" dot dot-variant="running"></cts-badge>
    </div>
  `,

  async play({ canvasElement }) {
    const [interactiveBadge, runningBadge] = canvasElement.querySelectorAll(".badge");

    // Interactive ring + dot together.
    expect(interactiveBadge.classList.contains("is-interactive")).toBe(true);
    expect(window.getComputedStyle(interactiveBadge).boxShadow).not.toBe("none");
    expect(interactiveBadge.querySelector(".cts-badge-dot-fail")).toBeTruthy();

    // Spinner + dot together.
    expect(runningBadge.querySelector(".cts-badge-spin")).toBeTruthy();
    expect(runningBadge.querySelector(".cts-badge-dot-running")).toBeTruthy();
  },
};
