...
/** Surface-colored gap between touching marks / around point rings. */
 ...
 * @type {Array<string>}
 */
const PLOT_PROPS = ["labels", "datasets", "stacked", "horizontal", "maxBars", "_status"];
 ...
 * it registers on `window`. Memoized; the memo is cleared on rejection so
 ...
 * no value is reachable only by hovering or by discriminating a fill color.
 *
...
class CtsChart extends LitElement {
  ...
  /** Create the chart, or push new data/options into the existing one. */
  _syncChart() {
    ...
    if (this._chart) {
      this._chart.data = this._buildData(cs);
   ...
   * rounded data-end, 2px surface gap between touching fills).
   * @param {CSSStyleDeclaration} cs - Computed style of `document.documentElement`.
    ...
    return {
      ...
      datasets: (this.datasets || []).map((ds) => {
        ...
        return {
          ...
          // The "border" is painted in the surface color: it IS the 2px gap
      ...
      plugins: {
        legend: {
          ...
          labels: {
            color: tickColor,
            font,
            // The legend key mirrors the mark it stands for: a rect, not a point.
            usePointStyle: false,
            boxWidth: 12,
            boxHeight: 12,
          },
  ...
  render() {
    ...
    const ariaLabel =
      `${this.heading}: bar chart of ${plotted} ${unit} across ` +
...
/**
 * TEST-ONLY: discard the memoized Chart.js loader promise so the next
