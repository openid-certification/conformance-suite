...
export const Default = {
  ...
  async play({ canvasElement, step }) {
    ...
    await step("trigger advertises the menu-button relationship", async () => {
    ...
    await step("menu is labelled by the trigger", async () => {
    ...
    await step("all items render and stay out of the page tab order", async () => {
...
export const PopoverOpens = {
  ...
  async play({ canvasElement, step }) {
    ...
    await step("popover opens as a menu", async () => {
      const popover = canvasElement.querySelector('[data-testid="overflow-popover"]');
      expect(popover.matches(":popover-open")).toBe(true);
      expect(popover.getAttribute("role")).toBe("menu");
    });

    await step("trigger reflects the open state", async () => {
    ...
    await step("first item receives focus", async () => {
...
export const TriggerArrowDownOpensAndFocusesFirst = {
  ...
  async play({ canvasElement, step }) {
    ...
    await step("ArrowDown on the closed trigger opens the menu", async () => {
      trigger.focus();
    ...
    await step("focus moves to the first item", async () => {
      await waitFor(() => {
        const items = canvasElement.querySelectorAll(".overflowItem");
...
export const TriggerArrowUpOpensAndFocusesLast = {
  ...
  async play({ canvasElement, step }) {
    ...
    await step("ArrowUp on the closed trigger opens the menu", async () => {
      trigger.focus();
    ...
    await step("focus moves to the last item", async () => {
      await waitFor(() => {
        const items = canvasElement.querySelectorAll(".overflowItem");
...
export const TabClosesMenu = {
  ...
  async play({ canvasElement, step }) {
    ...
    await step("Tab inside the open menu closes the popover", async () => {
    ...
    await step("trigger reflects the closed state", async () => {
      await waitFor(() => {
        expect(trigger.getAttribute("aria-expanded")).toBe("false");
      });
    });
...
export const ActionFiresEvent = {
  ...
  async play({ canvasElement, step }) {
    ...
    await step("clicking an item fires cts-overflow-action with its id", async () => {
      /** @type {Array<string>} */
      const fired = [];
    ...
    await step("activation closes the popover", async () => {
      await waitFor(() => {
        const popover = canvasElement.querySelector('[data-testid="overflow-popover"]');
        expect(popover.matches(":popover-open")).toBe(false);
      });
...
export const KeyboardArrowNavigation = {
  ...
  async play({ canvasElement, step }) {
    ...
    await step("ArrowDown then ArrowUp steps focus down and back", async () => {
      await userEvent.keyboard("{ArrowDown}");
      await waitFor(() => {
    ...
    await step("End then Home jump to the bookends", async () => {
...
export const PopoverClosesViaApi = {
  ...
  async play({ canvasElement, step }) {
    ...
    await step("popover starts open", async () => {
      expect(popover.matches(":popover-open")).toBe(true);
    });

    await step("hidePopover() closes it and surfaces the closed state", async () => {
      popover.hidePopover();
      await waitFor(() => {
        expect(popover.matches(":popover-open")).toBe(false);
      });
