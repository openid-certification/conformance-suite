 ...
 *
 * Slicing is the SERVER's job: the range presets and the
 ...
 * defense against a truncated or mocked payload (a permissive test route
 ...
 * colors (the same ones `cts-badge` and `cts-plan-status` paint), so the
...
/**
 * The per-family maps the payload carries, one per family chart.
 * @typedef {"testRunsByFamily"|"plansByFamily"|"certifiedByFamily"} FamilySeriesKey
 */

/**
 * Which per-family map each family chart and `otherBreakdown` read. An
 * explicit lookup rather than a caller-supplied key so a typo cannot reach
 * into the payload.
 * @type {Record<string, FamilySeriesKey>}
 */
...
export function rangeToQuery(range, now = new Date()) {
  ...
  // Date.UTC normalizes a negative or out-of-range month, so this crosses
...
export function visibleVariants(options, state) {
  ...
  for (const name of Object.keys(variants)) {
    ...
    if (text(filter[name]) || (narrowed && values.length > 1)) visible[name] = values;
  }
  return visible;
}

// --- Color slots and datasets -----------------------------------------

/**
 * Map every family to the color it wears everywhere on the page.
 *
 ...
 * color is an identity, so changing the range or any filter must never
...
/**
 * One of the payload's per-family series per period, one dataset per family,
 * colored with the same slots on every family chart so a family reads
 * identically on all of them.
 * @param {StatisticsData} data - The payload.
 * @param {Record<string, string>} slots - From {@link assignFamilySlots}.
 * @param {string} family - Selected family, or `""` for all.
 * @param {FamilySeriesKey} key - Which map to read.
 * @returns {Array<ChartDataset>} One dataset per rendered family.
 */
export function familyDatasets(data, slots, family, key) {
 ...
 * bucket order, wearing the suite's reserved status colors.
 *
...
export function memoizeByArgs(compute) {
  ...
  return /** @type {F} */ (/** @type {unknown} */ (memoized));
}
 ...
 *
 * Pure, and meant to be called through {@link memoizeByArgs}: the arrays it
...
export function buildChartInputs(data, slots, family) {
  ...
  return {
    ...
    runs: foldSeries(familyDatasets(data, slots, family, BREAKDOWN_SOURCES.runs)),
    plans: foldSeries(familyDatasets(data, slots, family, BREAKDOWN_SOURCES.plans)),
    certified: foldSeries(familyDatasets(data, slots, family, BREAKDOWN_SOURCES.certified)),
 ...
 * needs no legend: these are nominal categories, and coloring them by their
...
export function moduleDatasets(modules, ranking, metric, limit = MODULE_LIMIT) {
  // An unrecognized metric plots runs rather than nothing; the key is also the
 ...
 * server ranked on.
 *
 * Pure, and meant to be called through {@link memoizeByArgs}: what it returns
...
/**
 * @param {number} scaled - A byte count already divided down to `unit`.
 * @param {number} unit - Index into the byte units.
 * @returns {string} Whole bytes, one decimal for every larger unit.
 */
function shown(scaled, unit) {
...
export function formatBytes(bytes) {
  ...
  let scaled = value;
  let unit = 0;
  // Step on what would be PRINTED, not the raw value: 1023.96 KB rounds to
  // "1024.0 KB", which is a number no unit should show.
  while (unit < BYTE_UNITS.length - 1 && Number(shown(scaled, unit)) >= BYTE_STEP) {
    scaled /= BYTE_STEP;
    unit += 1;
  }
  return `${shown(scaled, unit)} ${BYTE_UNITS[unit]}`;
}
