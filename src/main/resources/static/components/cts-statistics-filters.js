...
const STYLE_TEXT = css`
  ...
  .cts-stats-range button:focus-visible {
    ...
    /* The ring overlaps the neighboring buttons, which would otherwise paint
  ...
  /* Plan names run to forty characters and are what the reader is choosing
     between, so this one select is not clipped to the width of the others. */
...
class CtsStatisticsFilters extends LitElement {
  ...
  constructor() {
    ...
    this._visibleVariants = memoizeByArgs(
      (
    ...
    this._sortedPlans = memoizeByArgs((/** @type {any} */ plans) =>
      Array.isArray(plans)
   ...
   *
   * Memoized on the payload's own list, whose identity only changes when a
