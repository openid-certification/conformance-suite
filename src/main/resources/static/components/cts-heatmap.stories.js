...
export const Default = {
  ...
  async play({ canvasElement, step }) {
    ...
    await step("the grid is one labeled image, not 168 announced boxes", async () => {
    ...
    await step("every swatch is labeled with the value it stands for", async () => {
      const scale = /** @type {HTMLElement} */ (host.querySelector(".cts-heatmap-scale"));
      // The ramp is square-root scaled, so labeling only the two ends would
