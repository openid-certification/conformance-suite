...
// page shows plus the neutral "Other" bucket — the shape the page renders.
const MONTHS = [
  "2025-12",
  "2026-01",
  "2026-02",
...
// tooltipFooter contract the page uses so a reader can see what "Other" means
...
export const Default = {
  ...
  render: ({ stacked, heading, categoryLabel }) => html`
    <cts-chart
      ?stacked=${stacked}
      heading=${heading}
  ...
  async play({ canvasElement, step }) {
    ...
    await step("chart is a stacked bar with a legend for its three series", async () => {
      ...
      // Bar borders are painted in the surface color: they ARE the 2px gap
      ...
      // solid surface-colored band across every bar.
