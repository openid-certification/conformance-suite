...
/**
 * `flashHighlight()` paints the Basecamp-style scroll-in wash. The host page
 * calls it after scrolling the cascade into view on a search-list selection.
 * With motion allowed, the `.oidf-spec-cascade--highlight` modifier is applied,
 * the decorative `::after` carries `pointer-events:none` / `z-index:-1` so the
 * selects stay legible and clickable, and cleanup runs on `animationend`.
 */
export const ScrollInHighlightFlash = {
  ...
  async play({ canvasElement }) {
    ...
    try {
      const wrapper = canvasElement.querySelector(".oidf-spec-cascade");
      expect(wrapper.classList.contains("oidf-spec-cascade--highlight")).toBe(false);

      element.flashHighlight();

      // The modifier lands after the restart-from-zero rAF.
      await waitFor(() => {
        expect(wrapper.classList.contains("oidf-spec-cascade--highlight")).toBe(true);
      });

      // The wash is non-interactive and sits behind the controls.
      const after = getComputedStyle(wrapper, "::after");
      expect(after.pointerEvents).toBe("none");
      expect(after.zIndex).toBe("-1");

      // Animation completion clears the modifier.
      wrapper.dispatchEvent(
        new AnimationEvent("animationend", {
          animationName: "oidf-cascade-flash",
          bubbles: true,
        }),
      );
      await waitFor(() => {
...
/**
 * Re-selecting while a flash is still in flight restarts it from zero (a fresh
...
export const HighlightRestartsOnReselect = {
  ...
  async play({ canvasElement }) {
    ...
    try {
      ...
      // not stuck off from the false→true toggle.
      element.flashHighlight();
      await waitFor(() => {
        expect(wrapper.classList.contains("oidf-spec-cascade--highlight")).toBe(true);
      });
