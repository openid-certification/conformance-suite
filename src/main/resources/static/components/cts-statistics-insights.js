 ...
 * hands it the same (memoized) distribution, so a re-render for the busy flag
 * cannot churn it.
...
function extraFooter(distribution) {
  ...
  return memoByIdentity(EXTRA_FOOTERS, distribution, () => (/** @type {number} */ index) => {
 ...
 * memoized `ModuleChart` until the payload changes, so `<cts-chart>` keeps
...
function moduleFooter(chart) {
  return memoByIdentity(
    MODULE_FOOTERS,
    chart,
    () => (/** @type {number} */ index) => (chart.footers || [])[index] || [],
  );
}

/**
 * The callback built from an object, once per object: the page hands this
 * component the same memoized inputs until the payload changes, so
 * `<cts-chart>` keeps being handed the same callback and a busy-flag
 * re-render re-plots nothing.
 * @template T
 * @param {WeakMap<object, T>} cache - Callbacks by the object they were built from.
 * @param {object} key - The object.
 * @param {() => T} build - Builds the callback on the first call for `key`.
 * @returns {T} The callback.
 */
function memoByIdentity(cache, key, build) {
...
const STYLE_TEXT = css`
  ...
  /* The variant band spans the whole row and lays its charts out itself: half
     ...
     almost-empty neighbor, and small multiples are meant to be read across. */
 ...
 * hands it slices of one payload it has already memoized, so a re-render for
...
class CtsStatisticsInsights extends LitElement {
   ...
   * here reaches both and the two cannot drift apart.
   * @param {string} heading - The chart's title.
   * @param {string} categoryLabel - What one bar is, for the table's first column.
   * @param {Distribution} distribution - Its inputs.
