	...
	.oidf-test-selector__family option:checked {
     ...
     browser's blue/gray system highlight — matching the .is-active row. */
		background: var(--orange-50)
			linear-gradient(0deg, var(--orange-50), var(--orange-50));
		color: var(--fg);
		font-weight: var(--fw-medium);
	}
	.oidf-test-selector__search-wrap {
...
class CtsTestSelector extends LitElement {
	...
	get _families() {
		const families = new Set(
			this.plans.map((p) => p.specFamily).filter(Boolean),
		);
	...
	get _filteredPlans() {
		...
		if (this._favoritesView) {
			...
			filtered = this.plans.filter(
				(p) => p.specFamily === this._selectedFamily,
			);
		} else {
			filtered = this.plans;
		}
	...
	get _hasLiveFavorites() {
		return (
			this._favoritesView &&
			this.favorites.some((name) =>
				this.plans.some((p) => p.planName === name),
			)
		);
	}
	...
	get _staleFavorites() {
		...
		let stale = this.favorites.filter(
			(name) => !this.plans.some((p) => p.planName === name),
		);
		if (this._searchTerm) {
			const term = this._searchTerm.toLowerCase();
	...
	_rowIndexFromEvent(event) {
		const raw = /** @type {HTMLElement} */ (event.currentTarget).dataset
			.index;
	...
	_handleRowKeydown(e) {
		...
		if (key === "f" || key === "F") {
			...
			if (plan) {
				this._emitFavoriteToggle(
					plan.planName,
					!this._isFavorite(plan.planName),
					"keyboard",
				);
			}
			return;
	...
	_handleRowClick(e) {
		...
		const planName = /** @type {HTMLElement} */ (e.currentTarget).dataset
			.planName;
	...
	_persistFilter() {
		...
		if (!storage) return;
		const filter = this._favoritesView
			? FAVORITES_FILTER_SENTINEL
			: this._selectedFamily;
		try {
	...
	updated(changed) {
		...
		if (changed.has("_focusedRowIndex") && this._focusedRowIndex >= 0) {
			...
			const rows = list
				? list.querySelectorAll(".oidf-test-selector__row")
				: [];
			const target = /** @type {HTMLElement | undefined} */ (
				rows[this._focusedRowIndex]
			);
			...
			<div class="oidf-test-selector">
				...
				<div class="oidf-test-selector__main">
					<div class="oidf-test-selector__list" role="list">
						...
						this._staleFavorites.length > 0
							? html`
									${this._filteredPlans.map((plan, index) =>
										this._renderRow(plan, index),
									)}
									${this._staleFavorites.map((name) =>
										this._renderStaleFavorite(name),
									)}
								`
							: this.loading
								? html`<cts-loading-state
										label="Loading test plans"
									></cts-loading-state>`
				...
				>
					<span class="oidf-test-selector__row-head">
						<span class="oidf-test-selector__row-title">
							...
							${plan.specFamily
								? html`<span
										class="oidf-test-selector__row-family"
						...
						${plan.modules
							? html`<cts-tooltip
									...
									><span
										...
										aria-label="${plan.modules
											.length} test ${plan.modules
											.length === 1
											? "module"
											: "modules"}"
				...
				<span class="oidf-test-selector__row-head">
					<span class="oidf-test-selector__row-title">
						<strong class="oidf-test-selector__row-name"
							>${name}</strong
						>
						<span class="oidf-test-selector__row-family"
	...
	_renderFamilyOptions() {
		return this._families.map(
			(f) =>
				html`<option
					value="${f}"
					?selected=${this._selectedFamily === f}
				>
					${f}
				</option>`,
		);
	...
	_renderListEmpty() {
		if (this._favoritesView) {
			...
			if (this.favorites.length === 0) {
				message = html`<strong>No favorites yet</strong> — star a plan
					to add it here.`;
