 ...
 * @param {Record<string, string>} slots - Family → color token.
 ...
 * @property {boolean} [stacked] - Whether the series stack: one bar per period, split by family.
...
const STYLE_TEXT = css`
  ...
  /* cts-empty-state centers its own content; the action under it has to be
     centered too, or it reads as belonging to whatever comes next. */
...
class CtsStatisticsPage extends LitElement {
  ...
  constructor() {
    ...
    this._familyOptions = memoizeByArgs(familiesWithActivity);
   ...
   * canceled it: `window.location.assign` cannot be stubbed reliably, so
  ...
  _drillDown(detail, family, certified = false) {
    ...
    if (!url) {
      // "Other" is a fold of the families outside the seven color slots, and
      // the two synthetic buckets are "these runs have no plan in the
      // registry" — neither is something the listing can be asked for.
      const refused = drillDownRefusal(this._state, click);
      ctsToast({
        title: `No drill-down for “${refused.family}”`,
   ...
   * payload, the color slots and the family filter, so a re-render that
   ...
   * @param {any} view - The memoized series for the current slice.
   ...
   *
   * All-time and unfiltered, unlike the tiles above: the server does not
   * scope this list by the query, so narrowing the range does not hide the
   * diagnostic the admin came for.
   * @param {StatisticsData} data - The current payload.
