 ...
 * where the family colors and the family select's options come from, so that
...
export const FiltersCascade = {
  ...
  async play({ canvasElement, step }) {
    ...
    await step("picking a family filters on the server", async () => {
      ...
      // ...and the users chart is filtered the same way.
...
/**
 * The unfiltered, whole-history view IS the baseline the colors and the
...
export const WholeHistoryNeedsNoBaseline = {
  ...
  async play({ canvasElement, step }) {
    ...
    await step("the family options and the colors come from it all the same", async () => {
...
export const RefreshingSnapshot = {
  ...
  async play({ canvasElement, step }) {
    ...
    await step("Refresh dims the charts instead of tearing them down", async () => {
      ...
      // after the first payload and may legitimately re-color the charts.
      await delay(300);
      plots = 0;
      await userEvent.click(button);
      await waitFor(() => {
