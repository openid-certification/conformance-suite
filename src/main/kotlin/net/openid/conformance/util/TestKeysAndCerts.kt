...
object TestKeysAndCerts {
	...
	// Self-signed IACA root, ECDSA P-256, expires 2027-08-03. Before regenerating (keep the
	// key, mint a new self-signed cert) check the result against ISO/IEC 18013-5 Table B.1;
	// the current cert satisfies it (critical basicConstraints CA:true/pathlen:0, critical
	// keyUsage keyCertSign+cRLSign, SKI, issuerAltName email, countryName + CN,
	// ECDSA-with-SHA256). The mattr checker at https://tools.mattrlabs.com/pem is useful.
	// Note: verifiers under test pin this certificate as a trust anchor — regenerating it
	// invalidates their configuration, so keep the same key and announce the change.
