...
-----END PRIVATE KEY-----"""
	...
	val documentSignerKey: AsymmetricKey.X509CertifiedExplicit
		get() = rotatingDocumentSignerKey.get()
	...
	val statusListSignerKey: AsymmetricKey.X509CertifiedExplicit
		get() = rotatingStatusListSignerKey.get()
	...
	/**
	 * A leaf key that is re-minted daily (and so never served anywhere near its certificate's
	 * expiry): frequent rotation keeps regeneration a routine event, so nothing can quietly start
	 * depending on one specific leaf certificate — the IACA root is the only stable thing to pin.
	 */
	private class DailyRotatedKey(private val mint: () -> AsymmetricKey.X509CertifiedExplicit) {
		private var cached: AsymmetricKey.X509CertifiedExplicit? = null
		private var mintedAt: kotlin.time.Instant? = null

		@Synchronized
		fun get(): AsymmetricKey.X509CertifiedExplicit {
			val current = cached
			val at = mintedAt
			if (current != null && at != null && Clock.System.now() < at + 1.days) {
				return current
			}
			return mint().also {
				cached = it
				mintedAt = Clock.System.now()
			}
		}
	}

	private val rotatingDocumentSignerKey = DailyRotatedKey { mintDocumentSignerKey() }
	private val rotatingStatusListSignerKey = DailyRotatedKey { mintStatusListSignerKey() }
	 ...
	 * a critical key usage asserting digitalSignature only, and a 90-day validity, well under
	 * the maxima of Table B.3 (457 days) and Table B.9 (1187 days). [addProfileExtensions] adds
	 * whatever else the specific leaf profile calls for.
	 */
	...
	private fun mintStatusListSignerKey(): AsymmetricKey.X509CertifiedExplicit =
		mintLeafUnderIaca("certification.openid.net mdoc status list signer") { }
	 ...
	 * point (mandatory in Table B.3, though the suite does not operate a CRL).
	 */
	private fun mintDocumentSignerKey(): AsymmetricKey.X509CertifiedExplicit =
		mintLeafUnderIaca("certification.openid.net mdoc DS") { builder ->
			builder.addExtension(
