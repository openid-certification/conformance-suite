...
//	}
	protected VPResponseModeVerifierTests getEffectiveResponseModeVariant() {
		return getVariant(VPResponseModeVerifierTests.class);
	}
	protected VPRequestMethodVerifierTests getEffectiveClientRequestTypeVariant() {
		return getVariant(VPRequestMethodVerifierTests.class);
	}
...
//
//		if(clientRequestType == VPRequestMethod.REQUEST_OBJECT || clientRequestType == ClientRequestType.REQUEST_URI) {
...
//		boolean fireTestFinishedCalled = false;
	...
	protected void validateClientMetadata() {
		...
		callAndContinueOnFailure(OIDCCValidateClientRedirectUris.class, ConditionResult.FAILURE,
			"OIDCR-2");
	...
	protected void fetchClientJwksFromJwksUri() {
		skipIfElementMissing("client", "jwks_uri", ConditionResult.INFO, FetchClientKeys.class,
			ConditionResult.FAILURE, "OIDCC-10.1.1", "OIDCC-10.2.1");
	}
	...
	protected void encryptIdTokenIfNecessary() {
		skipIfElementMissing("client", "id_token_encrypted_response_alg", ConditionResult.INFO,
			EncryptIdToken.class, ConditionResult.FAILURE, "OIDCC-10.2");
	}
...
*/
	...
	protected void addCHashToIdToken() {
		skipIfMissing(null, new String[] { "c_hash" }, ConditionResult.INFO,
			AddCHashToIdTokenClaims.class, ConditionResult.FAILURE, "OIDCC-3.3.2.11");
	}

	protected void addAuthTimeToIdToken() {
		skipIfElementMissing("effective_authorization_endpoint_request", "max_age", ConditionResult.INFO,
			AddAuthTimeToIdTokenClaims.class, ConditionResult.FAILURE, "OIDCC-3.1.2.1");
	}
...
//			callAndStopOnFailure(EnsureRequestDoesNotContainRequestObject.class, "OIDCC-6.1");
	...
	protected void extractNonceFromAuthorizationEndpointRequestParameters() {
		...
		if (responseType != null && responseType.contains("id_token")) {
			...
			callAndContinueOnFailure(ExtractNonceFromAuthorizationRequest.class, ConditionResult.INFO, "OIDCC-3.1.2.1");
		}
	}
	...
	protected void validateAuthorizationEndpointRequestParameters() {
		...
		// FIXME not sure what validation to do here? maybe response_uri??
//		callAndContinueOnFailure(EnsureValidRedirectUriForAuthorizationEndpointRequest.class, ConditionResult.FAILURE,"OIDCC-3.1.2.1");
	...
	protected void validateRequestObject() {
		...
		if(allowUnsignedRequestObjects() && "none".equals(alg)) {
			...
			skipIfMissing(new String[]{"client_public_jwks"}, null, ConditionResult.FAILURE,
				ValidateRequestObjectSignature.class, ConditionResult.FAILURE, "OIDCC-6.1");
		}
	}
	...
	protected void validateResponseTypeAuthorizationRequestParameter() {
		callAndStopOnFailure(EnsureResponseTypeIsVpToken.class);
	}
...
//		createIdToken(false); // FIXME sd_jwt...
