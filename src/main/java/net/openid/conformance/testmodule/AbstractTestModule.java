...
public abstract class AbstractTestModule implements TestModule, DataUtils {
	...
	private void setStatusInternal(Status newStatus) {
		try {
			...
			// fully-consistent snapshot (status + statusUpdated). Null-safe because the first
			// setStatusInternal(CREATED) call fires inside setProperties (line 129) before
			// TestRunner has wired the service.
