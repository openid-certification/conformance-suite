...
public interface TestInfoRepository extends PagingAndSortingRepository<TestInfo, String>, CrudRepository<TestInfo, String> {
	...
	Slice<InternalTestInfo> findAllBy(Pageable pageable);

	@Query(value = "{ $text: { $search: ?0 } }", count = true)
	Long countAllSearch(String search);

	@Query("{ $text: { $search: ?0 } }")
	Slice<InternalTestInfo> findAllSearch(String search, Pageable pageable);

	@Query("{ owner: ?0 }")
	Iterable<InternalTestInfo> findAllByOwner(Map<String, String> owner);

	@Query(value = "{ owner: ?0 }", count = true)
	Long countAllByOwner(Map<String, String> owner);

	@Query("{ owner: ?0 }")
	Slice<InternalTestInfo> findAllByOwner(Map<String, String> owner, Pageable pageable);
	...
	Slice<InternalTestInfo> findAllByOwnerSearch(Map<String, String> owner, String search, Pageable pageable);
	...
	@Query("{ _id: ?0, owner: ?1 }")
	Optional<InternalTestInfo> findByIdAndOwner(String id, Map<String, String> owner);
