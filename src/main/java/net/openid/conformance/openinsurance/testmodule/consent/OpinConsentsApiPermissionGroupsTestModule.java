...
@PublishTestModule(
	testName = "opin-consent-api-test-permission-groups-v1",
	displayName = "Validate that consent API V1 accepts the consent groups",
	summary = "Validates that consent API V1 accepts the consent groups\n" +
		...
		"\u2022 Validates consent API V1 request for 'Personal Registration Data' permission group(s)\n" +
		"\u2022 Validates consent API V1 request for 'Personal Additional Information' permission group(s)\n" +
		"\u2022 Validates consent API V1 request for 'Business Registration Data' permission group(s)\n" +
		"\u2022 Validates consent API V1 request for 'Business Additional Information' permission group(s)\n" +
		"\u2022 Validates consent API V1 request for 'Accounts Balances' permission group(s)\n" +
		"\u2022 Validates consent API V1 request for 'Accounts Limits' permission group(s)\n" +
		"\u2022 Validates consent API V1 request for 'Credit Card Invoices' permission group(s)\n" +
		"\u2022 Validates consent API V1 request for 'Credit Operations' permission group(s)\n" +
		"\u2022 Validates consent API V1 request for 'Balances & Credit Card Limits' permission group(s)\n" +
		"\u2022 Validates consent API V1 request for 'Accounts Transactions' permission group(s)\n" +
		"\u2022 Validates consent API V1 request for 'Credit Card Limits' permission group(s)\n" +
		"\u2022 Validates consent API V1 request for 'Credit Card Transactions' permission group(s)\n",
...
public class OpinConsentsApiPermissionGroupsTestModule extends AbstractClientCredentialsGrantFunctionalTestModule {
	...
	private void validatePermissions(String[] permissions, String name) {
		String logMessage = String.format("Validate consent API V1 request for '%s' permission group(s)", name);
