...
@PublishTestModule(
	testName = "fapi1-advanced-final-par-ensure-client-assertion-with-wrong-sub-fails",
	displayName = "FAPI1-Advanced-Final: PAR: ensure client_assertion with wrong sub fails",
	summary = "This test sends a client assertion with a wrong 'sub' value (not equal to the client_id) to the PAR endpoint. Per RFC 9126 section 2, authentication at the PAR endpoint must follow the same rules as the token endpoint. Per RFC7523 section 3 and OIDC Core section 9 the sub claim must contain the client_id. The server must reject this request.",
...
public class FAPI1AdvancedFinalPAREnsureClientAssertionWithWrongSubFails extends AbstractFAPI1AdvancedFinalServerTestModule {
	...
	protected void addClientAuthenticationToPAREndpointRequest() {
		...
		call(new CreateJWTClientAuthenticationAssertionAndAddToPAREndpointRequest()
			.insertAfter(CreateClientAuthenticationAssertionClaims.class,
				condition(SetSubToWrongValueInClientAssertionClaims.class).requirements("RFC7523-3", "OIDCC-9")));
		unmapClientAuthKeys();
	}
