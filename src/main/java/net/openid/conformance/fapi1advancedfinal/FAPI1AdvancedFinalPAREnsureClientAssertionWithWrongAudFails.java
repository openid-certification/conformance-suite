...
@PublishTestModule(
	testName = "fapi1-advanced-final-par-ensure-client-assertion-with-wrong-aud-fails",
	displayName = "FAPI1-Advanced-Final: PAR: ensure client_assertion with wrong aud fails",
	summary = "This test sends a client assertion with a wrong 'aud' value to the PAR endpoint. Per RFC 9126 section 2, the authorization server must accept its issuer identifier, token endpoint URL, or pushed authorization request endpoint URL as valid audience values, but must reject any other value. The server must reject this request.",
...
public class FAPI1AdvancedFinalPAREnsureClientAssertionWithWrongAudFails extends AbstractFAPI1AdvancedFinalServerTestModule {
	...
	protected void addClientAuthenticationToPAREndpointRequest() {
		...
		call(new CreateJWTClientAuthenticationAssertionAndAddToPAREndpointRequest()
			.replace(UpdateClientAuthenticationAssertionClaimsWithISSAud.class,
				condition(AddWrongAudToClientAssertionClaims.class).requirements("PAR-2", "RFC7523-3")));
		unmapClientAuthKeys();
	}
