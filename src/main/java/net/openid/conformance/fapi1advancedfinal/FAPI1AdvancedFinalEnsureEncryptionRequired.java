...
@PublishTestModule(
	...
	summary = "This test makes a FAPI authorization request via the front channel without encrypting the request object, which the Open Banking Brazil, Open Insurance Brazil and KSA security profiles say must be rejected. An invalid_request_object or access_denied error may be returned from the authorization endpoint, or an error may be shown to the user (a screenshot of which must be uploaded).",
...
// Only profiles that require an encrypted request object, and only when it is passed by value
@VariantNotApplicable(parameter = FAPIAuthRequestMethod.class, values = {
	"pushed"
})
@VariantApplicableOnly(parameter = FAPI1FinalOPProfile.class, values = { "openbanking_brazil", "openinsurance_brazil", "openbanking_ksa" })
public class FAPI1AdvancedFinalEnsureEncryptionRequired extends AbstractFAPI1AdvancedFinalExpectingAuthorizationEndpointPlaceholderOrCallback {
