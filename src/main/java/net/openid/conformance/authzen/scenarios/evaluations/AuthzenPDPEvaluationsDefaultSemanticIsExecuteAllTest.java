...
@PublishTestModule(
	...
	displayName = "Authzen Evaluations API - Section 7.1.2.1: Default evaluations_semantic is execute_all",
	summary = "Per Section 7.1.2.1, when the request omits `options.evaluations_semantic` the PDP MUST process every evaluation and return a decision for each one (`execute_all` semantic). Sends a mixed permit/deny batch and asserts both decisions are returned in order.\n" + AuthzenPDPEvaluationsDefaultSemanticIsExecuteAllTest.payload,
	profile = "Authzen"
)
