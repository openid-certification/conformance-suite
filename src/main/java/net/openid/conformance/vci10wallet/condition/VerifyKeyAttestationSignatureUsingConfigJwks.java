	...
	public Environment evaluate(Environment env) {
		...
		try {
			...
			if (!(walletPublicKey instanceof ECKey ecPublicKey)) {
				String errorDescription = "Key attestation signing key in 'Key Attestation JWKS' is not an EC key";
				VCICredentialErrorResponseUtil.updateCredentialErrorResponseInEnv(env, VciErrorCode.INVALID_PROOF, errorDescription);
				throw error(errorDescription, args("kty", walletPublicKey.getKeyType()));
			}
			if (!Curve.P_256.equals(ecPublicKey.getCurve())) {
				String errorDescription = "Key attestation signing key in 'Key Attestation JWKS' does not use the required P-256 curve";
				VCICredentialErrorResponseUtil.updateCredentialErrorResponseInEnv(env, VciErrorCode.INVALID_PROOF, errorDescription);
