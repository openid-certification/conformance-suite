		...
		if (body.has("localInstrument") && OIDFJSON.getString(body.get("localInstrument")).equals("INIC")) {
			assertField(body,
				new StringField
					...
					.setMaxLength(25)
					.build());
		} else if (body.has("localInstrument") && (OIDFJSON.getString(body.get("localInstrument")).equals("MANU")) || body.has("localInstrument") && (OIDFJSON.getString(body.get("localInstrument")).equals("DICT"))) {
			assertField(body,
				new StringField
					.Builder("transactionIdentification")
					.setPattern("^(?![\\s\\S])")
