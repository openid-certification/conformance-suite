...
@ApiName("Car Insurance List")
public class CarInsuranceListValidator extends AbstractJsonAssertingCondition {
	...
	public CarInsuranceListValidator() {
	...
	private void assertProducts(JsonObject products) {
		...
		assertField(products,
			new StringField
				.Builder("autoInsuranceAdditionalInfo")
				.setMaxLength(100)
				.setOptional()
				.build());
	...
	private void assertCoverages(JsonObject coverages) {
		...
		assertField(coverages,
			new ObjectField
				.Builder("newCar")
				.setValidator(newCar -> assertField(newCar,
					new ObjectArrayField
						.Builder("contractBase")
						.setValidator(this::assertContractBase)
						.setMinItems(1)
	...
	protected void assertHasDoubleField(JsonObject jsonObject, String path) {
		super.assertHasDoubleField(jsonObject, path);
	}
	...
	private void assertAttributes(JsonObject coverageAttributes) {
		...
		assertField(coverageAttributes,
			new ObjectField
				...
				.setValidator(mandatoryParticipation -> {

					assertField(mandatoryParticipation,
						new StringField
							.Builder("amount")
							.setMaxLength(300)
							.build());
					...
					assertField(mandatoryParticipation,
						new ObjectField
							.Builder("unit")
							.setValidator(this::assertCoverageItemAttributeUnit)
							.build());
				})
				.setOptional()
				.build());
	...
	private void assertCoverageItemAttributeUnit(JsonObject coverageItemAttributeUnit) {

		assertField(coverageItemAttributeUnit, Fields.code().setMaxLength(4).build());

		assertField(coverageItemAttributeUnit,
			new StringField
				.Builder("description")
				.setMaxLength(80)
				.build());
