 ...
 * The client should send a PAR request that updates an existing grant, i.e. grant_management_action=merge
 * or replace (GM 5.2) together with a grant_id.
...
@PublishTestModule(
	...
	summary = "Configure your client to send a PAR request that updates an existing grant, that is with grant_management_action set to merge or replace and a grant_id. The suite will return an invalid_grant_id error (HTTP 400) from the PAR endpoint. The test expects the client to display an error indicating that the grant_id was rejected, and to NOT proceed to the authorization endpoint.",
...
public class FAPI2SPFinalClientTestGrantManagementInvalidGrantIdFails extends AbstractFAPI2SPFinalClientTestGrantManagement {
	...
	/**
	 * This test asks the client to update an existing grant, so a grant_id must be present; the value
	 * itself is irrelevant as the suite rejects it regardless.
	 */
	@Override
	protected void checkGrantIdInParEndpointRequest() {
		// the inherited action check accepts any of the three actions; 'create' with a grant_id is a
		// different error case (GM 5.4 makes it invalid_request), so narrow it to merge/replace here
		callAndContinueOnFailure(GrantManagementSupport.EnsurePARRequestContainsGrantManagementActionUpdatingAGrant.class,
			ConditionResult.FAILURE, "GM-5.2");
		callAndContinueOnFailure(GrantManagementSupport.EnsurePARRequestContainsGrantId.class, ConditionResult.FAILURE, "GM-5.2");
	}

	@Override
	protected ResponseEntity<Object> createPAREndpointCustomErrorResponse() {
		callAndStopOnFailure(GrantManagementSupport.CreatePAREndpointInvalidGrantIdErrorResponse.class, "GM-5.4");
