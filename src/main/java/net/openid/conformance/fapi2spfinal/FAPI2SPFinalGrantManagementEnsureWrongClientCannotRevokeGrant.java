...
// because it needs the multiple-client base class, so the exclusion is declared inline here.
...
public class FAPI2SPFinalGrantManagementEnsureWrongClientCannotRevokeGrant extends AbstractFAPI2SPFinalMultipleClient {
	...
	/** This module calls the grant management endpoint, so it needs the GM 6.1 grant management API scopes. */
	@Override
	...
	protected void onPostAuthorizationFlowComplete() {
		if (!isSecondClient()) {
			...
			callAndContinueOnFailure(GrantManagementSupport.EnsureGrantManagementEndpointReturns403Or404.class, Condition.ConditionResult.FAILURE, "GM-6.6");
			eventLog.endBlock();
