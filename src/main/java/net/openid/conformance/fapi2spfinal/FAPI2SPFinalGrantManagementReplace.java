...
public class FAPI2SPFinalGrantManagementReplace extends AbstractFAPI2SPFinalGrantManagementTestModule {
	...
	protected void onPostAuthorizationFlowComplete() {
		if (!secondAuthFlowDone) {
			...
			// a 'replace' updates the grant the request named, so the AS must not have minted a new one
			...
			// the grant the AS reports has to cover the access it just issued, otherwise the 'replace' did not
			// take effect
