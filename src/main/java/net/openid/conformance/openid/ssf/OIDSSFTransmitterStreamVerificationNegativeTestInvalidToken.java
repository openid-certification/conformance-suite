...
@PublishTestModule(
	testName = "openid-ssf-transmitter-stream-verification-error-invalid-token",
	displayName = "Attempt to trigger a Stream Verification with invalid access token.",
	summary = """
		This test verifies that the transmitter rejects a verification request with an invalid
		access token (SSF 1.0 8.1.4.2, Table 10: "401 if authorization failed or it is missing").
		The testsuite expects to observe the following interactions:
		 * create a stream
		 * attempt to trigger a verification event with an invalid access token
...
public class OIDSSFTransmitterStreamVerificationNegativeTestInvalidToken extends AbstractStreamControlErrorTest {
	...
	protected void testTransmitter() {
		...
		eventLog.runBlock("Attempt to trigger a verification event with invalid access token", () -> {
			...
			call(exec().mapKey("endpoint_response", "resource_endpoint_response_full"));
			callAndContinueOnFailure(EnsureHttpStatusCodeIs401.class, Condition.ConditionResult.FAILURE, "OIDSSF-8.1.4.2");
