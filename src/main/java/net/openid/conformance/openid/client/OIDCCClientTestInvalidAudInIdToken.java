...
public class OIDCCClientTestInvalidAudInIdToken extends AbstractOIDCCClientTestExpectingNothingInvalidIdToken {
	...
	protected Object authorizationCodeGrantType(String requestId) {
		if(responseType.includesIdToken()) {
			throw new ConditionError(getId(), "Client has incorrectly called token_endpoint after receiving an id_token with an invalid aud claim from the authorization_endpoint.");
		} else {
	...
	{
		throw new ConditionError(getId(), "Client has incorrectly called userinfo_endpoint after receiving an id_token with an invalid aud claim.");
	}
}
