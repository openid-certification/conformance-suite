...
@PublishTestModule(
	testName = "fapi1-advanced-final-br-v1-par-ensure-pkce-required",
	displayName = "FAPI1-Advanced-Final-Br-v1: ensure pkce is required when using pushed authorization requests",
	summary = "This test makes a FAPI authorization request without using PKCE (RFC7636), which must be rejected. FAPI1-Advanced-Final-Br-v1 requires servers to reject PAR requests that do not use PKCE, clause 5.2.2-18. Depending on when the server chooses to verify the request, the refusal may be an error from the pushed authorization request endpoint, or an invalid_request error may be returned from the authorization endpoint, or an error may be shown to the user (a screenshot of which must be uploaded).",
...
public class FAPI1AdvancedFinalBrV1PAREnsurePKCERequired extends AbstractFAPI1AdvancedFinalBrV1ExpectingAuthorizationEndpointPlaceholderOrCallback {
