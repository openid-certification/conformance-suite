...
public class FAPI2SPID2AustraliaConnectIdTestClaimsParameterIdTokenIdentityClaims extends AbstractFAPI2SPID2ServerTestModule {
	...
	protected ConditionSequence makeCreateAuthorizationRequestSteps() {
		...
		return super.makeCreateAuthorizationRequestSteps()
			...
			.then(condition(AustraliaConnectIdAddVerifiedClaimsToAuthorizationEndpointRequestIdTokenClaims.class)
				...
				.skipIfStringMissing("au_connectid_claims_supported")
	...
	protected void exchangeAuthorizationCode() {
		...
		// Check the trust_framework only if id_token verified claims were requested.
		call(condition(AustraliaConnectIdEnsureIdTokenContainsTrustFramework.class)
			.requirements("CID-IDA-5.2-11")
			.skipIfElementMissing("authorization_endpoint_request", "claims.id_token.verified_claims.claims")
			.onSkip(Condition.ConditionResult.INFO)
			.onFail(Condition.ConditionResult.FAILURE));

		// Check the verified_claims  only if id_token verified claims were requested.
		call(condition(AustraliaConnectIdEnsureIdTokenContainsVerifiedClaims.class)
