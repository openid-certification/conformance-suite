...
//		callAndStopOnFailure(CheckServerConfiguration.class); // FIXME doesn't like the openid4vp:// url being set as authorization endpoint url
	...
	private void submitMockWalletResponseIfConfigured() {
		...
		getTestExecutionManager().runInBackground(() -> {
			...
			if (origin != null) {
				env.getObject("client").addProperty("client_id", "origin:" + origin);
			}
			...
			if (responseMode == VP1FinalWalletResponseMode.DC_API_JWT) {
				// VP1FinalEncryptVPResponse reads encryption params from authorization_request_object.
				// For unsigned DC API requests there's no request object, so set one up from the
				// direct request parameters.
				if (!env.containsObject("authorization_request_object")) {
					JsonObject fakeRequestObject = new JsonObject();
					JsonObject claims = env.getObject("authorization_endpoint_request").deepCopy();
					fakeRequestObject.add("claims", claims);
					env.putObject("authorization_request_object", fakeRequestObject);
				}
