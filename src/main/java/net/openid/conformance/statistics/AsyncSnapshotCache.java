 ...
 * is ever in flight, so any number of polling callers cause at most one computation. A
 * forced call that arrives while one is running has it run once more when it finishes: the
 * running one may have read the data before the change the caller is refreshing for.
 *
...
public class AsyncSnapshotCache<T> {
	...
	/** A forced call arrived while a computation was running; run again once it finishes. */
	private boolean rerunRequested;

	/**
	 * @param computation    produces a snapshot; runs on {@code executor}, may throw anything
	 ...
	 *              has not expired; while a computation is running this has it run once
	 *              more when it finishes rather than starting a second one
	...
	public synchronized State<T> get(boolean force) {
		...
		} else if (shouldStart(force, now)) {
			inFlightStartedAt = now;
			inFlight = CompletableFuture.runAsync(this::runUntilSettled, executor);
			running = true;
		}
		if (value != null) {
	...
	private boolean shouldStart(boolean force, Instant now) {
		...
		return force || (stale && backoffExpired);
	}

	private void runUntilSettled() {
		do {
			runOnce();
		} while (takeRerunRequest());
	}

	private synchronized boolean takeRerunRequest() {
		boolean requested = rerunRequested;
	...
	private void runOnce() {
		...
		try {
			...
			// Throwable, not Exception: an OutOfMemoryError while building a snapshot of a large
			// database must count as a failure too, or the next poll starts another
			// computation straight away, with no log line and outside the backoff
