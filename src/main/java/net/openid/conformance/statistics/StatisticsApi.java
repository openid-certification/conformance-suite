...
public class StatisticsApi {
	...
	@Operation(operationId = "getStatisticsOverview",
		...
		description = "Served from a snapshot that is recomputed in the background at most every 12 hours, "
			...
			+ "the granularity, and honors `family` and `plan` as registry membership - a module belongs to "
			...
			+ "`data.topUsers` lists the 20 users who ran the most test modules, counted over the same runs as "
			+ "`data.modules` and so windowed and filtered the same way. It is the one part of the payload that "
			+ "identifies users, by `iss` and `sub`.\n\n"
	...
	// The return type is deliberately left unparameterized. Gson serializes a body by its
	...
	// declares no fields - so anything that gets Gson to serialize AGAINST the declared
	// type instead (a parameterized wrapper, or a direct Gson.toJson(body, type)) turns
	// every response into "{}".
