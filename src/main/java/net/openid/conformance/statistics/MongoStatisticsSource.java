...
public class MongoStatisticsSource {
	...
	/**
	 * The host part of a URL: what follows the scheme and any userinfo, up to the port, path,
	 * query or fragment. An IPv6 literal is taken whole, brackets included, since the colons
	 * inside it are not a port separator.
	 */
	static final String HOST_OF_URL = "^https?://(?:[^/?#@]*@)?(\\[[^\\]]*\\]|[^/:?#]+)";
	 ...
	 * {@value StatisticsCube#HOST_MONTHS} months, by descending run count. The window has
	 * its own constant because this is the snapshot's most expensive scan: it reads every
	 * run's configuration, which is most of a {@code TEST_INFO} document, so it is the one
	 * to shorten first.
	 *
	...
	public TileRow tiles(Instant now, Instant serverStartedAt, long totalUsers) {
		...
		String cutoff30d = cutoff.toString();
		String sinceServerStart = serverStartedAt.toString();
		Instant oldest = serverStartedAt.isBefore(cutoff) ? serverStartedAt : cutoff;
	 ...
	 * <em>inside the server</em>, so the plans' {@code config} blobs are never materialized
	 ...
	 *         none. The candidates are normalized <em>before</em> the fallback rather than
	...
	private static Document targetUrl() {
		Document normalize = new Document("$cond", List.of(
		...
		Document candidates = new Document("$map",
			new Document("input", TARGET_URL_FIELDS).append("in", normalize));
