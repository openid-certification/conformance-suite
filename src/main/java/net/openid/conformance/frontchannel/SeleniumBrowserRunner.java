...
/**
 * The original {@link BrowserRunner}: a headless HtmlUnit browser driven through Selenium's
 * {@link HtmlUnitDriver}. This is the default engine (see {@link BrowserControl}).
 *
 * <p>All HtmlUnit instances created for one test module instance share the {@link CookieManager}
 * owned by the {@link BrowserControl}, which the OIDC prompt=login tests rely on.
 */
public class SeleniumBrowserRunner implements BrowserRunner, DataUtils {
	...
	private final boolean cssParsingEnabled;

	private final BrowserVisit visit;
	private final ResponseCodeHtmlUnitDriver driver;
	 ...
	 * @param cookieManager     cookie store shared between all runners of the test module instance
	 * @param verboseLogging    log every HTTP request/response and every browser console message
	 * @param cssParsingEnabled whether HtmlUnit should parse CSS (see options.browsercontrol_css_enable)
	 * @param visit             the url, tasks and options to run
	 */
	SeleniumBrowserRunner(BrowserControl browserControl, String testId, TestInstanceEventLog eventLog,
						  CookieManager cookieManager, boolean verboseLogging, boolean cssParsingEnabled,
						  BrowserVisit visit) {
		...
		this.cssParsingEnabled = cssParsingEnabled;
		this.visit = visit;

		// each runner gets it's own driver... that way two could run at the same time for the same test.
				...
				try {
					if (elementType.equalsIgnoreCase("contains")) {
						...
						if (updateImagePlaceHolder || updateImagePlaceHolderOptional) {
							...
							browserControl.updatePlaceholder(visit.placeholder(), driver.getPageSource(), driver.getResponseContentType(), regexp, updateImagePlaceHolderOptional);
						}
					} else {
	...
	public JsonObject getStatus() {
		...
		o.addProperty("currentUrl", driver.getCurrentUrl());
		o.addProperty("currentTask", currentTask);
		o.addProperty("currentCommand", currentCommand);
		o.addProperty("lastResponseCode", driver.getResponseCode());
		o.addProperty("lastResponseContentType", driver.getResponseContentType());
		o.addProperty("lastResponseContent", driver.getResponseContent());
