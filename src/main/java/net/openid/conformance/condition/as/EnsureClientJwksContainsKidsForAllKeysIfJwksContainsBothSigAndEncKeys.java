 ...
 * ...
 */
public class EnsureClientJwksContainsKidsForAllKeysIfJwksContainsBothSigAndEncKeys extends AbstractCondition
{
	...
	public Environment evaluate(Environment env) {
		...
		try
		{
			...
			if(totalKeyCount == keysWithKidsCount) {
				logSuccess("All keys have key ids");
				return env;
			}
			if(sigKeyCount == 0 && encKeyCount == 0) {
				logSuccess("No keys found, nothing to have a key id");
				return env;
			}
			if(sigKeyCount > 0 && encKeyCount > 0)
			{
				...
				logSuccess("Jwks is not required to have kids because it only contains signing keys");
			} else {
				logSuccess("Jwks is not required to have kids because it only contains encryption keys");
			}
		}
