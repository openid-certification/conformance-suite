...
public class EnsureMatchedRicalEntryHasNoTrustConstraints extends AbstractRicalCondition {
	...
	public Environment evaluate(Environment env) {
		...
		// The governing entry is read from the RICAL, so the list's COSE signature is verified
		// first: a corrupted RICAL must not decide whether constraints apply, whatever severity
		// this condition runs at. Same policy as ValidateRequestObjectX5cChainAgainstRical.
		...
		try {
			...
			throw error("The configured RICAL could not be parsed or its COSE signature does not verify, so the matched entry's trust constraints cannot be examined", e);
		}

		if (!verifyChainAgainstRical(signedRical, chainCerts).isTrusted()) {
			// F.3.2.6 applies trust constraints as part of validating a chain that reaches a
			// listed CA; an untrusted chain has no governing entry, and is reported by
			// ValidateRequestObjectX5cChainAgainstRical
			log("The request object certificate chain does not validate against the RICAL, so no entry governs its trust constraints");
			return env;
		}
