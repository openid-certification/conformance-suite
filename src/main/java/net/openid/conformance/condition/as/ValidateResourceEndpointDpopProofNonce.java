...
public class ValidateResourceEndpointDpopProofNonce extends AbstractValidateDpopProofNonce {

	@Override
	@PreEnvironment(required = {"incoming_dpop_proof", "incoming_request"})
//	@PostEnvironment(strings = "resource_endpoint_dpop_nonce_error")  // optional variable set
	public Environment evaluate(Environment env) {
		final String ERROR_KEY = "resource_endpoint_dpop_nonce_error";
		env.removeNativeValue(ERROR_KEY);
		String expectedNonce = env.getString("resource_server_dpop_nonce"); // check for server side saved nonce
