...
public class GrantManagementSupport {
	 ...
	 * The quality of the value is checked separately by {@link CheckGrantIdIsUrlSafeWithSufficientEntropy}.
	 */
	...
	/**
	 * Checks the quality of the issued grant_id: GM 5.5 requires it to be "a URL safe string" with
	 * "sufficient entropy to prevent guessing". The spec gives no minimum length, so the length check
	 * below is a heuristic - callers must invoke this with WARNING severity rather than failing an AS
	 * whose identifiers are shorter than the suite's rule of thumb but still unguessable.
	 */
	public static class CheckGrantIdIsUrlSafeWithSufficientEntropy extends AbstractCondition {

		// base64url alphabet; anything outside it has to be percent encoded to appear in the grant resource URL
		...
		public Environment evaluate(Environment env) {
			...
			if (!URL_SAFE.matcher(grantId).matches()) {
				throw error("grant_id contains characters that are not URL safe; GM 5.5 requires the grant_id to be a URL safe string, as it forms the last path segment of the grant resource URL",
			...
			logSuccess("grant_id is a URL safe string of a plausible length", args("grant_id", grantId));
			return env;
		}
	}
	 ...
	 * callers invoke this with FAILURE severity, unlike
	 * {@link CheckDiscoveryForGrantManagementActionsSupported}, whose absence is only advisory.
	 */
	...
	public static class CallGrantManagementEndpointQuery extends AbstractCallGrantManagementEndpoint {
		...
		/**
		 * The body has to be parsed here, as that is what produces body_json for the conditions that check
		 * the contents of the response; those checks remain separate so their severity stays with the
		 * caller. {@link CallGrantManagementEndpointFailureQuery} turns this off for the negative paths,
		 * where the response is not expected to be JSON at all.
		 */
		@Override
	...
	/**
	 * As {@link CallGrantManagementEndpointQuery}, but for the cases where the query is expected to fail and
	 * hence the response body is not necessarily JSON.
	 */
	...
	public static class ValidateGrantManagementQueryResponseTimestamps extends AbstractGrantManagementQueryResponseCondition {
		...
		public Environment evaluate(Environment env) {
			...
			Long lastUpdatedAt = validateTimestamp(body, "last_updated_at", now);
			...
			if (lastUpdatedAt != null && lastUpdatedAt > now + CLOCK_SKEW_SECONDS) {
				throw error("'last_updated_at' in the grant management query response is in the future",
					args("last_updated_at", lastUpdatedAt, "now", now));
			}

			if (createdAt != null && lastUpdatedAt != null && lastUpdatedAt < createdAt) {
				throw error("'last_updated_at' in the grant management query response is before 'created_at'",
					args("created_at", createdAt, "last_updated_at", lastUpdatedAt));
			}
			...
			logSuccess("The timestamps in the grant management query response are consistent", args("body", body));
			return env;
		}
	...
	public static class EnsurePARRequestDoesNotContainGrantIdWithCreateAction extends AbstractCheckGrantManagementParametersInPARRequest {
		...
		public Environment evaluate(Environment env) {
			...
			if ("create".equals(action) && grantId != null) {
				throw error("The pushed authorization request contains a grant_id alongside grant_management_action=create; GM 5.2 requires that grant_id MUST NOT be present for the create action",
	...
	public static class AddGrantManagementToServerConfiguration extends AbstractCondition {
		...
		public Environment evaluate(Environment env) {
			...
			String grantManagementEndpoint = tokenEndpoint.replaceFirst("(token)$", "grants");
			...
			if (server.has("mtls_endpoint_aliases")) {
				...
				mtlsAliases.addProperty("grant_management_endpoint", mtlsTokenEndpoint.replaceFirst("(token)$", "grants"));
			}
			...
			log("Added grant management metadata to the server configuration",
