...
// Adds an nbf of 62 seconds in the future to the Client Authentication Assertion.
public class AddNbfOver60SecondsInTheFutureToClientAuthenticationAssertionClaims extends AbstractCondition {
	...
	public Environment evaluate(Environment env) {
		...
		Instant nbf = Instant.now().plusSeconds(62);

		claims.addProperty("nbf", nbf.getEpochSecond());

		env.putObject("client_assertion_claims", claims);

		logSuccess("Added nbf value to client assertion claims which is 62 seconds in the future", args("client_assertion_claims", claims, "nbf_is_62_seconds_in_the_future", nbf));
