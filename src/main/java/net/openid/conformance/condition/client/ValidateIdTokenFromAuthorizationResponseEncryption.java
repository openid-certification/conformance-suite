...
public class ValidateIdTokenFromAuthorizationResponseEncryption extends AbstractVerifyJweEncryption {

	@Override
	@PreEnvironment(required = { "authorization_endpoint_response", "client_jwks" })
	public Environment evaluate(Environment env) {

		JsonObject clientJwks = env.getObject("client_jwks");

		JsonElement tokenElement = env.getElementFromObject("authorization_endpoint_response", "id_token");
		if (tokenElement == null || !tokenElement.isJsonPrimitive()) {
			throw error("Couldn't find id_token in authorization_endpoint_response");
		}
