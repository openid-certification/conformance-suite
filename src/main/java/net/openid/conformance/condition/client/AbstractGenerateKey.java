	...
	protected JWK createJwkForAlg(Environment env, String alg) {
		try {
			return switch (alg) {
				...
				case "Ed25519" -> onConfigureOkp(new OctetKeyPair.Builder(PreGeneratedJwks.nextOkpKey(env, Curve.Ed25519))
					.keyUse(KeyUse.SIGNATURE).algorithm(JWSAlgorithm.Ed25519)).build();
