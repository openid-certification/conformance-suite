...
public class RegisterVical extends AbstractCondition {
	...
	public Environment evaluate(Environment env) {
		...
		// the setup sequence runs once per validated credential; only register on the first run
		if (env.getString("vical_setup_complete") != null) {
			log("VICAL setup already ran for this test, skipping");
			return env;
		}
		env.putString("vical_setup_complete", "true");
		// guards the one-shot VICAL quality checks; the setup sequence clears it when done
		env.putString("vical_checks_pending", "true");
