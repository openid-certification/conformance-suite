...
public class WarnIfRequestObjectClaimsBindingMessageIsNotAscii extends AbstractCondition {

	@Override
	@PreEnvironment(required = "request_object_claims")
	public Environment evaluate(Environment env) {
		JsonElement bindingMessageElement = env.getElementFromObject("request_object_claims", "binding_message");

		if (bindingMessageElement == null) {
			throw error("'binding_message' in request object claims is required but not present");
		}
		if (!bindingMessageElement.isJsonPrimitive() || !bindingMessageElement.getAsJsonPrimitive().isString()) {
			throw error("'binding_message' in request object claims is not a string");
		}
