...
public class ValidateRicalStructure extends AbstractRicalCondition {
	...
	// F.3.2.2's CDDL spells the constraints key "trustContraints" while its own field list says
	// "trustConstraints". Only the correct spelling is accepted here: a list using the CDDL's
	// typo has its constraints ignored by every parser that follows the field list (multipaz
	// included), so reporting the key as unknown is what tells the tester their constraints
	// are invisible.
	private static final Set<String> KNOWN_CERTIFICATE_INFO_KEYS = Set.of(
		...
		"name", "issuingCountry", "stateOrProvinceName", "issuer", "subject",
	...
	private void checkTopLevel(DataItem rical, List<String> findings) {
		...
		// however the absolute checks above come out
		if (date != null && notAfter != null && notAfter < date) {
			findings.add("'notAfter' is before 'date' - the RICAL expired before it was issued");
		}
		if (date != null && nextUpdate != null && nextUpdate < date) {
			findings.add("'nextUpdate' is before 'date' - the next RICAL was due before this one was issued");
		}
	...
	private void checkTrustAnchorReachability(List<DataItem> entries, List<String> findings) {
		...
		for (int i = 0; i < certs.size(); i++) {
			...
			String prefix = "certificateInfos[" + i + "] (" + cert.getSubject().getName() + "): ";
			List<X509Cert> candidateIssuers = listedIssuerCandidates(cert, certs);
			if (candidateIssuers.isEmpty()) {
				findings.add(prefix + "has isTrustAnchor false but its issuer is not listed in the RICAL, so it has no path to a trust anchor entry");
				continue;
			}
			if (!signedByAnyOf(cert, candidateIssuers)) {
				findings.add(prefix + "has isTrustAnchor false and names a listed entry as its issuer, but its signature does not verify with that entry's key, so it has no path to a trust anchor entry");
			}
		}
	}

	/**
	 * The listed certificates that could have issued the given certificate: those whose Subject
	 * Key Identifier matches its Authority Key Identifier, or, for a certificate without an
	 * AKI, those whose subject name matches its issuer name. A subject name alone can be shared
	 * by an old and a renewed CA, or by an unrelated CA, so a name match is only a fallback and
	 * {@link #signedByAnyOf} decides the link either way.
	 */
	private static List<X509Cert> listedIssuerCandidates(X509Cert cert, List<X509Cert> listed) {
		byte[] aki = cert.getAuthorityKeyIdentifier();
		String issuerName = cert.getIssuer().getName();
		List<X509Cert> candidates = new ArrayList<>();
		for (X509Cert other : listed) {
			if (other.equals(cert)) {
				continue;
			}
			boolean matches = aki != null
				? Arrays.equals(other.getSubjectKeyIdentifier(), aki)
				: other.getSubject().getName().equals(issuerName);
			if (matches) {
	...
	private static boolean signedByAnyOf(X509Cert cert, List<X509Cert> candidateIssuers) {
		for (X509Cert issuer : candidateIssuers) {
			try {
				kotlinx.coroutines.BuildersKt.runBlocking(
					kotlin.coroutines.EmptyCoroutineContext.INSTANCE,
					(scope, continuation) -> cert.verify(issuer.getEcPublicKey(), continuation));
				return true;
			} catch (Exception e) {
				// not issued by this candidate; the finding is raised if none verifies
			}
		}
		return false;
	}
