...
makeVcTests() {
    ...
    TESTS="${TESTS} oid4vp-1final-verifier-test-plan[$VPPROFILE][$SDJWT][$SANDNS][$SIGNEDREQ][$DIRECTPOST]:oid4vp-1final-verifier-happy-flow{oid4vp-1final-wallet-test-plan[$VPPROFILE][$SDJWT][$SANDNS][$SIGNEDREQ][$DIRECTPOST]:oid4vp-1final-wallet-alternate-happy-flow}${CONFIGS}/vp-wallet-test-config-dcql-sdjwt.json ${CONFIGS}/vp-verifier-test-config-with-redirect.json"
    ...
    # The issuer multiple-clients module needs the emulated issuer to serve a second client, and in the
    # issuer-initiated flows to offer it a credential of its own (issueToSecondClient in the wallet config)
    TESTS="${TESTS} oid4vci-1_0-wallet-test-plan[$SIMPLE][$CLIATTAUTH][$UNSIGNED][$DPOP][$WALLETINIT][$AUTHCODE][$OFFERBYVALUE][$HAIP][$SDJWTVC][$CRED_RESP_ENCR_PLAIN][$ISSUANCE_IMMEDIATE]:oid4vci-1_0-wallet-test-credential-issuance{oid4vci-1_0-issuer-test-plan[$SIMPLE][$CLIATTAUTH][$UNSIGNED][$DPOP][$WALLETINIT][$AUTHCODE][$HAIP][$SDJWTVC][$CRED_RESP_ENCR_PLAIN][$FAPI_OPENID][$FAPI_RESPONSEMODE]:oid4vci-1_0-issuer-happy-flow-multiple-clients}${SUITE_DIR}/scripts/test-configs-rp-against-op/vci-issuer-test-config-client_attestation-client-auth-dpop.json ${SUITE_DIR}/scripts/test-configs-rp-against-op/vci-two-clients-wallet-test-config-haip.json"
    TESTS="${TESTS} oid4vci-1_0-wallet-test-plan[$SIMPLE][$CLIATTAUTH][$UNSIGNED][$DPOP][$ISSUERINIT][$AUTHCODE][$OFFERBYVALUE][$HAIP][$SDJWTVC][$CRED_RESP_ENCR_PLAIN][$ISSUANCE_IMMEDIATE]:oid4vci-1_0-wallet-test-credential-issuance{oid4vci-1_0-issuer-test-plan[$SIMPLE][$CLIATTAUTH][$UNSIGNED][$DPOP][$ISSUERINIT][$AUTHCODE][$HAIP][$SDJWTVC][$CRED_RESP_ENCR_PLAIN][$FAPI_OPENID][$FAPI_RESPONSEMODE]:oid4vci-1_0-issuer-happy-flow-multiple-clients}${SUITE_DIR}/scripts/test-configs-rp-against-op/vci-issuer-test-config-client_attestation-client-auth-dpop.json ${SUITE_DIR}/scripts/test-configs-rp-against-op/vci-two-clients-wallet-test-config-haip.json"
    TESTS="${TESTS} oid4vci-1_0-wallet-test-plan[$SIMPLE][$MTLSAUTH][$UNSIGNED][$DPOP][$ISSUERINIT][$PREAUTHCODE][$OFFERBYVALUE][$PLAINVCI][$SDJWTVC][$CRED_RESP_ENCR_PLAIN][$ISSUANCE_IMMEDIATE]:oid4vci-1_0-wallet-test-credential-issuance{oid4vci-1_0-issuer-test-plan[$SIMPLE][$MTLSAUTH][$UNSIGNED][$DPOP][$ISSUERINIT][$PREAUTHCODE][$PLAINVCI][$SDJWTVC][$CRED_RESP_ENCR_PLAIN][$FAPI_OPENID][$FAPI_RESPONSEMODE]:oid4vci-1_0-issuer-happy-flow-multiple-clients}${SUITE_DIR}/scripts/test-configs-rp-against-op/vci-issuer-test-config-client_attestation-client-auth-dpop.json ${SUITE_DIR}/scripts/test-configs-rp-against-op/vci-two-clients-wallet-test-config-plain.json"
