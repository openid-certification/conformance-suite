...
makeSsfTests() {
    ...
    TESTS="${TESTS} openid-ssf-receiver-test-plan[$PUSH_DELIVERY][$CAEP_INTEROP_PROFILE][$SSF_AUTH_MODE_DYNAMIC][$CLIENT_AUTH_CLIENT_SECRET_POST]:openid-ssf-receiver-happypath{openid-ssf-transmitter-test-plan[$PUSH_DELIVERY][$CAEP_INTEROP_PROFILE][$STATIC_CLIENT][$SERVER_METADATA_DISCOVERY][$CLIENT_AUTH_CLIENT_SECRET_POST][$SSF_METADATA][$SSF_AUTH_MODE_DYNAMIC]:openid-ssf-transmitter-metadata,openid-ssf-stream-control-error-read-stream-without-token,openid-ssf-stream-control-error-token-in-uri-query,openid-ssf-stream-control-error-create-stream-with-read-only-token,openid-ssf-stream-control-happy-path}${SUITE_DIR}/scripts/test-configs-ssf/ssf-transmitter-test-config-dynamic.json ${SUITE_DIR}/scripts/test-configs-ssf/ssf-receiver-test-config-dynamic.json"
