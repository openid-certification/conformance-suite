...
env_list = pp.Group(pp.OneOrMore(env_param)).set_results_name("enviroment")
env_list.setParseAction(lambda t: {item[0]: item[1] for item in t.as_list()[0]})
module_list = pp.Group(colon + pp.delimitedList(name, ',')).set_results_name('modules')
test_with_var_and_modules = name.set_results_name("test_name") + pp.Opt(variant_list) + pp.Opt(
    module_list) + pp.Opt(env_list)
test_with_var_and_modules.setParseAction(lambda t : t.as_dict())
op_test = left_curly + test_with_var_and_modules.setDebug(debug_enabled)("test_plan") + right_curly + pp.Opt(path).setDebug(
    debug_enabled).set_results_name("config")
op_test.setParseAction(lambda t : t.as_dict())
test_unit = test_with_var_and_modules.set_results_name("test") + pp.Opt(op_test).set_results_name("op_test")
test_unit.setParseAction(lambda t : t.as_dict())
test_plan = pp.Group(
...
def parse_test(test_string):
    ...
    test_unit.skipWhitespace = False
