...
def patch_config(config_path: str, jwk: dict) -> None:
    """Patch a VP test config file with the new JWK."""
    with open(config_path) as f:
        config = json.load(f)

    if "credential" in config and "signing_jwk" in config["credential"]:
        config["credential"]["signing_jwk"] = jwk
    if "client" in config and "jwks" in config["client"]:
        config["client"]["jwks"] = {"keys": [jwk]}
...
def main():
    ...
    parser.add_argument("--patch", action="append", default=[], help="Config file to patch with new JWK")
    ...
    for config_path in args.patch:
        patch_config(config_path, jwk)
        print(f"Patched {config_path}", file=sys.stderr)
