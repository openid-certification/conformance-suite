...
def run_tests():
    ...
    # authenticated. The admin API token must still satisfy that, and must still
    ...
    resp = admin_client.get(f"{base_url}api/currentuser")
    runner.check_status("Authority: admin API token reaches the API chain", resp, 200)
    if resp.status_code == 200:
        display_name = resp.json().get("displayName")
        runner.check("Authority: admin API token resolves to a display name",
