...
./scripts/run-integration-tests.sh --fapi-tests
./scripts/run-integration-tests.sh --oidcc-tests
./scripts/run-integration-tests.sh --ciba-tests
...
There is no dedicated `--vp-tests` option. To run only VP plans from `--server-tests-only`, temporarily comment out the OID4VCI `TESTS=` lines inside `makeServerTest` in `.gitlab-ci/run-tests.sh`, run the script, then restore the file.
